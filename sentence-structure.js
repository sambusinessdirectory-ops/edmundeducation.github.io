...
import { SENTENCE_REALMS } from "./sentence-structure-realms.mjs?v=20260915-blinktrophy1";
import { SENTENCE_MAP_LIMIT, sentenceMapLessons, sentenceMapCompleted } from "./sentence-structure-map.mjs?v=20260914-hotel3b";
import { GOLDEN_EDDIE_ART, sentenceTrophyState, sentenceTrophyCollection, goldenEddieFigure, renderSentenceTrophyShelf, syncSentenceMapTrophies, syncSentenceTrophyCounter, syncSentenceTrophyControls, animateSentenceTrophy, awardDateMarkup } from "./sentence-structure-trophies.mjs?v=20260915-blinktrophy1";
...
function currentProgressQuestionId(lesson = getLesson()) {
  ...
  const questions = orderQuestions(state.exercise.correctionMode ? correctionQuestions(lesson) : lesson.questions || [],{system:'sentence-structure',owner:state.user?.id,lessonId:state.lessonId});
...
function questionHtml(question) {
  ...
  return `<article class="question-card ${correct ? "is-correct" : wrong ? "is-wrong" : ""} ${collapsed ? "is-collapsed" : ""}" data-question-id="${escapeHtml(question.id)}" data-edmund-prompt-text="${escapeHtml(question.prompt || question.english || "")}"${revealAnswer ? ` data-edmund-answer-text="${escapeHtml(voiceAnswer)}"` : ""} data-edmund-record-id="${escapeHtml(question.id)}" data-edmund-record-title="${escapeHtml(`句子結構 · 第 ${question.number || ""} 題`)}">
    ...
    <div class="question-card-content" ${collapsed ? "hidden" : ""}>
      ...
      <p class="question-feedback" aria-live="polite">${correct ? answerComparison(value, question).typoCount === 1 ? "✓ 答案正確；黃色標示一個可留意的拼寫。" : "✓ 答案正確，這題已完成。" : wrong ? unresolvedCorrection ? "答案未完全符合句型；請再次修改後提交。" : "答案未完全符合句型；請參考答案並修改。" : ""}</p>
...
installQuestionOrder({system:'sentence-structure',owner:()=>state.user?.id,lessonId:()=>state.lessonId});
