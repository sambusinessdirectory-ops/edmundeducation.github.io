import { motivationRatingsCsv } from "./schedule-motivation.mjs?v=20260812-1";
...
async function exportCsv() {
  ...
  try {
    ...
    const blob = new Blob([motivationRatingsCsv(allRows)], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    const exportFilters = state.committedFilters || readFilters();
    link.download = `daily-motivation-${exportFilters.p_date_from}-${exportFilters.p_date_to}.csv`;
