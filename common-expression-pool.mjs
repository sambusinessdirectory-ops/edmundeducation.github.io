...
export function poolTerrain(){const nodes=poolPositions(),path=nodes.map((p,i)=>{const y=p.y-POOL_TOP;if(!i)return `M${p.x} ${y}`;const a=nodes[i-1];return a.x===p.x?`C${p.x+(p.x>800?110:-110)} ${a.y-POOL_TOP} ${p.x+(p.x>800?110:-110)} ${y} ${p.x} ${y}`:`L${p.x} ${y}`;}).join(' ');
 return `<section class="pool-realm" style="top:${POOL_TOP}px" aria-label="桌球旅程，平台 121 至 150"><img class="pool-background" src="${ART}table-v1.jpg" width="1600" height="1200" draggable="false" alt="胡桃木框綠絨桌球檯，球桿、三角球架與彩色桌球"><svg class="pool-route" viewBox="0 0 1600 1200" aria-hidden="true"><path d="${path}" fill="none" stroke="#062b2290" stroke-width="56" transform="translate(0 6)"/><path d="${path}" fill="none" stroke="#c0a260" stroke-width="55"/><path d="${path}" fill="none" stroke="#ead6a6" stroke-width="47"/><path d="${path}" fill="none" stroke="#fff6df99" stroke-width="2" stroke-dasharray="7 17"/></svg><canvas class="pool-motion" width="1600" height="1200" aria-hidden="true"></canvas>${nodes.map(p=>`<button type="button" class="pool-platform${p.striped?' is-striped':''}" data-pool-platform="${p.order}" style="left:${p.x}px;top:${p.y-POOL_TOP}px;--ball-color:${p.color}" aria-label="平台 ${p.order}，課題準備中"><span class="pool-ball"><span class="pool-ball-number">${p.order}</span></span><small>課題準備中</small></button>`).join('')}</section>`;
}
