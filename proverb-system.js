...
import { createExpressionMap } from './common-expression-map.mjs?v=20260923-pink-rain-jacket1';
 ...
 *
 * Lesson IDs are permanent `proverb-NN` identifiers. Every published lesson has
 * exactly 50 questions named `proverb-NN-q01` through `proverb-NN-q50`.
 ...
 * `highlights` metadata.
 */
const CONFIG = window.EDMUND_PROVERB_SYSTEM_CONFIG || {};
const SUPABASE_CONFIG = window.EDMUND_SUPABASE || {};
const CONTENT = window.EDMUND_PROVERB_SYSTEM_DATA || { version: "missing", lessons: [] };

const SESSION_KEY = "edmund-proverb-system-session-v1";
...
const DEFAULT_PAGE_META = Object.freeze({
  1: Object.freeze({ titleZh: "形式及例子", titleEn: "Form(s) + Example(s)", kicker: "MEANING + FORM", description: "先掌握諺語的核心意思、常用形式及例子。" }),
  2: Object.freeze({ titleZh: "語域及語氣", titleEn: "Register and Tone", kicker: "REGISTER + TONE", description: "了解這個諺語最自然的場合，以及何時應改用較正式的表達。" }),
  3: Object.freeze({ titleZh: "固定部分及可變部分", titleEn: "Fixed and Variable Parts", kicker: "WHAT STAYS + WHAT CHANGES", description: "分辨諺語中固定不變的字詞，以及可按語境調整的部分。" }),
  4: Object.freeze({ titleZh: "所有特定形式", titleEn: "Forms and Specific Variants", kicker: "USEFUL FORMS", description: "逐一掌握本課整理的常用形式、變化及搭配。" }),
  5: Object.freeze({ titleZh: "表達好處", titleEn: "Benefits", kicker: "WHY THIS PROVERB HELPS", description: "理解這個諺語能帶出的語意、語氣及溝通效果。" }),
  6: Object.freeze({ titleZh: "歷史及來源", titleEn: "History and Origin", kicker: "HISTORY + ORIGIN", description: "了解這個諺語的歷史背景、來源可信度及記憶提示。" }),
...
const elements = {
  ...
  username: document.querySelector("#proverb-system-username"),
  password: document.querySelector("#proverb-system-password"),
  ...
  loadingTemplate: document.querySelector("#proverb-system-loading-template"),
...
function validLessonContract(lesson) {
  if (!isPlainObject(lesson) || !/^proverb-\d{2}$/.test(String(lesson.id || ""))) return false;
  if (!String(lesson.titleZh || lesson.title || "").trim()) return false;
  if (!String(lesson.titleEn || lesson.englishTitle || "").trim()) return false;
  if (!lessonIllustration(lesson).src) return false;
  if (!Array.isArray(lesson.questions) || lesson.questions.length !== 50) return false;
...
function lessonList() {
  if (CONTENT.system !== "proverb" || !Array.isArray(CONTENT.lessons)) return [];
  const seen = new Set();
...
function lessonTitle(lesson) {
  return String(lesson?.title || lesson?.titleZh || lesson?.name || "英文諺語");
}

function lessonEnglishTitle(lesson) {
  return String(lesson?.titleEn || lesson?.englishTitle || "English Proverb");
}
...
function workerBaseUrl() {
  ...
  if (!baseUrl.startsWith("https://")) throw new Error("英文諺語服務尚未完成設定。");
  return baseUrl;
}
...
async function apiJson(path, options = {}, includeAuth = true, authToken = state.authToken) {
  ...
  try {
    ...
    const connectionError = new Error("暫時未能連接英文諺語服務，請檢查網絡後再試。");
...
async function adminLogin(username, password) {
  ...
  return {
    ...
    user: {
      id: String(admin.id || "proverb-system-admin"),
...
async function handleLogin(event) {
  ...
  try {
    ...
    console.warn("Proverb System login failed", error);
...
async function logout() {
  ...
  try {
    ...
    console.warn("Proverb System logout cleanup failed", error);
  }
  clearSession();
...
async function openDashboard({ force = false } = {}) {
  ...
  try {
    ...
    if (String(state.user?.id || "") !== userId || String(state.authToken || "") !== authToken) return;
    console.warn("Proverb System dashboard failed", error);
...
function openRequestedHomeworkLesson() {
  ...
  if (lessonId.length > 80 || !getLesson(lessonId)) {
    showToast("這個 Proverb System 練習目前不存在。", "error");
    return false;
  }
...
function renderLessonChoices() {
  ...
  const cards = lessonList().map((lesson, index) => {
    ...
    return `
      <article class="lesson-choice-card ${complete ? "is-complete" : ""}">
        ...
        <button class="lesson-section-bookmark" type="button" data-toggle-section-bookmark="${escapeHtml(lesson.id)}" aria-pressed="${bookmarked}" aria-label="${bookmarked ? "移除諺語書簽" : "收藏整個諺語"}">${bookmarked ? "★" : "☆"}</button>
      </article>
    `;
  }).join("");
  ...
  elements.lessonChoiceGrid.innerHTML = `<button class="lesson-choice" type="button" data-open-bookmarks-card data-number="★" data-tone="bookmark">
      ...
      <span class="choice-meta"><span>${escapeHtml(sectionBookmarkCount)} 個諺語</span><span>${escapeHtml(questionBookmarkCount)} 道題目</span><span>跟隨帳戶同步</span></span>
    </button>${cards}`;
}
...
function attemptHistoryHtml(attempts, { allowResume = true } = {}) {
  if (!attempts.length) return '<p class="empty-state">暫時未有練習記錄。完成或開始一組諺語練習後，記錄會顯示在這裡。</p>';
  return attempts.map((attempt) => {
...
function navHtml(page) {
  return `<div class="lesson-navigation">
    ${page > 1 ? `<button class="secondary-button" type="button" data-lesson-prev>← 上一頁</button>` : '<button class="secondary-button" type="button" data-back-to-dashboard>← 返回諺語選擇</button>'}
...
function renderFormulaPage(lesson) {
  ...
  elements.lessonContent.innerHTML = `<article class="info-page">
    ...
    <section class="formula-card">
      <span class="formula-label">諺語形式 · FORM</span>
    ...
    ${hasModelBreakdown ? `<section class="lesson-detail-section" aria-labelledby="model-example-heading">
      ...
      <div class="lesson-prose-grid">
        ...
        <article class="lesson-prose-card">
          <h3>諺語版本 <small>Proverb Version</small></h3>
...
function bilingualItem(item) {
  ...
  return {
    english: String(item?.english || item?.en || ""),
    chinese: String(item?.chinese || item?.zh || item?.text || ""),
...
function renderExercisePage(lesson, { preserveScroll = false } = {}) {
  ...
  const exerciseTitleZh = String(exerciseMeta.titleZh || "諺語句子改寫練習");
  const exerciseTitleEn = String(exerciseMeta.titleEn || "Proverb Sentence Practice");
  ...
  elements.lessonContent.innerHTML = `<section class="exercise-page">
    ...
    ${illustration.src ? `<figure class="exercise-proverb-illustration">
    ...
    ${visibleCorrectIds.length ? `<div class="question-list-toolbar">
      <button class="bulk-visibility-button" type="button" data-toggle-all-correct-cards aria-pressed="${allVisibleCorrectCollapsed}" aria-controls="proverb-system-question-list" aria-label="${bulkVisibilityLabel}（${escapeHtml(visibleCorrectIds.length)} 題）">${bulkVisibilityLabel}</button>
    </div>` : ""}

    <div class="question-list" id="proverb-system-question-list" data-question-list>
...
function scheduleExercisePersistence() {
  ...
  state.exercisePersistTimer = window.setTimeout(() => {
    ...
    persistExercise().catch((error) => {
      console.warn("Proverb System preference save failed", error);
    });
  }, 450);
}
...
async function submitExercise(kind) {
  ...
  try {
    ...
    console.warn("Proverb System attempt save failed", error);
  ...
  try {
    ...
    if (String(state.user?.id || "") === operationUserId) {
      showToast(existingIndex >= 0
        ? (sectionBookmark ? "已移除諺語書簽。" : "已移除題目書簽。")
        : (sectionBookmark ? "已收藏整個諺語。" : "已加入題目書簽。"));
    }
  } catch (error) {
    ...
    if (state.currentView === "bookmarks") renderBookmarks();
    console.warn("Proverb System bookmark sync failed", error);
  ...
  elements.bookmarkList.innerHTML = `<div class="bookmark-columns">
    <section class="bookmark-column" aria-labelledby="section-bookmark-heading">
      <header class="bookmark-column-heading">
        <span>01</span>
        <div><h2 id="section-bookmark-heading">收藏諺語</h2><p>整個英文諺語課題</p></div>
      </header>
      <div class="bookmark-column-list">${sectionRows || '<p class="empty-state">暫時未收藏諺語。可在學習首頁的諺語卡右上角按 ☆。</p>'}</div>
    </section>
    <section class="bookmark-column" aria-labelledby="question-bookmark-heading">
      <header class="bookmark-column-heading">
        <span>02</span>
        <div><h2 id="question-bookmark-heading">收藏題目</h2><p>各諺語內的個別練習題</p></div>
      </header>
...
async function openAdminDashboard() {
  ...
  try {
    ...
    console.warn("Proverb System admin students failed", error);
...
async function openAdminStudent(studentId) {
  ...
  try {
    ...
    console.warn("Proverb System admin student detail failed", error);
...
async function initialise() {
  ...
  if (!lessonList().length) {
    ...
    setStatus(elements.loginStatus, "英文諺語教材暫時未能載入，請重新整理頁面。", "error");
    elements.loginButton.disabled = true;
    return;
  }
  checkHealth();
...
initialise().catch((error) => {
  console.error("Proverb System initialisation failed", error);
  clearSession();
