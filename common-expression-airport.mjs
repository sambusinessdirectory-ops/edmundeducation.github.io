import {DINING_TOP,prepareDining,diningPositions,diningTerrain,mountDining} from './common-expression-dining.mjs?v=20260915-dining1';
import {TRAIN_TOP,prepareTrain,trainPositions,trainTerrain,mountTrain} from './common-expression-train.mjs?v=20260915-dining1';
...
export async function prepareAirport() {
  ...
  assets=Promise.all([image(ART+'lounge-v1.jpg'),image(ART+'sprites-v1.png'),prepareTrain(),prepareDining()]).then(([background,atlas])=>{
...
function areaFor(point){return point.y>=DINING_TOP?'dining':point.y>=TRAIN_TOP?'train':'airport';}
function areaTop(point){return point.y>=DINING_TOP?DINING_TOP:point.y>=TRAIN_TOP?TRAIN_TOP:0;}
function combinedPositions(lessons){const positions=[...airportPositions(lessons.slice(0,30)),...trainPositions(),...diningPositions()];return lessons.map((lesson,i)=>({...positions[i],id:lesson.id}));}
function combinedTerrain(nodes,lessons){return terrain(nodes.slice(0,30),lessons.slice(0,30))+trainTerrain()+diningTerrain()+[TRAIN_TOP,DINING_TOP].map(y=>`<div class="business-theme-mist" style="top:${y-60}px" aria-hidden="true"></div>`).join('');}
function combinedMount(root,reduced,controls){
  const airport=mount(root,reduced),train=mountTrain(root,reduced),dining=mountDining(root,reduced),events=new AbortController();
  ...
  tabs.innerHTML='<button type="button" data-business-area="airport" aria-pressed="true">機場貴賓室 · 01–30</button><button type="button" data-business-area="train" aria-pressed="false">臥鋪列車 · 31–60</button><button type="button" data-business-area="dining" aria-pressed="false">月夜餐車 · 61–90</button>';
  ...
  root.querySelector('.expression-map-heading small').textContent='26 個課題已開放 · 90 個旅程平台';
  let currentArea='';
  function choose(area,point){
    if(area!==currentArea){currentArea=area;root.dataset.businessArea=area;tabs.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.businessArea===area)));note.hidden=area==='airport';note.textContent=area==='dining'?'平台 61–90 · 課題準備中':'平台 31–60 · 課題準備中';}
    if(point&&controls)controls.explore(point);
  }
  tabs.addEventListener('click',e=>{const b=e.target.closest('[data-business-area]');if(b){const area=b.dataset.businessArea;choose(area,area==='dining'?diningPositions()[0]:area==='train'?trainPositions()[0]:airportPositions([{id:'first'}])[0]);}},{signal:events.signal});
  root.addEventListener('click',e=>{const b=e.target.closest('[data-train-platform],[data-dining-platform]');if(!b)return;e.stopImmediatePropagation();const order=Number(b.dataset.trainPlatform||b.dataset.diningPlatform),p=[...trainPositions(),...diningPositions()].find(p=>p.order===order);root.querySelectorAll('[data-train-platform],[data-dining-platform]').forEach(n=>n.setAttribute('aria-pressed',String(n===b)));note.textContent=`平台 ${p.order} · 課題準備中`;controls?.explore(p,{walk:true});},{capture:true,signal:events.signal});
  return {draw(time){const position=controls?.getPosition();if(position)choose(areaFor(position));const scale=Number(root.dataset.scale)||1,top=(viewport?.scrollTop||0)/scale,bottom=top+(viewport?.clientHeight||3600)/scale;if(top<TRAIN_TOP)airport.draw(time);if(top<DINING_TOP&&bottom>TRAIN_TOP)train.draw(time);if(bottom>DINING_TOP)dining.draw(time);},destroy(){events.abort();airport.destroy();train.destroy();dining.destroy();tabs.remove();note.remove();}};
}
function businessFloor(point){if(point.y>=DINING_TOP)return {x:clamp(point.x,360,1390),y:clamp(point.y,DINING_TOP+575,DINING_TOP+1100)};if(point.y>=TRAIN_TOP)return {x:clamp(point.x,210,1400),y:clamp(point.y,TRAIN_TOP+545,TRAIN_TOP+1140)};return floorPoint(point);}
export const BUSINESS_AIRPORT=Object.freeze({id:'airport',title:'商務會話探索之旅',kicker:'BUSINESS SPEAKING · JOURNEYS',width:W,height:H*3,positions:combinedPositions,terrain:combinedTerrain,mount:combinedMount,fitOverview:true,overviewBounds:point=>({top:areaTop(point),height:H,key:areaFor(point)}),minimumZoom:.7,cameraScaleFloor:()=>.55,cameraTop:({point,scale,height,overview})=>{const start=areaTop(point);return overview||point.y-start<790?start*scale:point.y*scale-height*.55;},navigation:{path:(_from,to)=>[businessFloor(to)],step:(_from,to)=>businessFloor(to)}});
