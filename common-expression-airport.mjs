import {LOUNGE_TOP,prepareLounge,loungePositions,loungeTerrain,mountLounge} from './common-expression-lounge.mjs?v=20260915-lounge3';
import {POOL_TOP,preparePool,poolPositions,poolTerrain,mountPool} from './common-expression-pool.mjs?v=20260915-lounge3';
...
export async function prepareAirport() {
  ...
  assets=Promise.all([image(ART+'lounge-v1.jpg'),image(ART+'sprites-v1.png'),prepareTrain(),prepareDining(),preparePoker(),preparePool(),prepareLounge()]).then(([background,atlas])=>{
...
function areaFor(point){return point.y>=LOUNGE_TOP?'lounge':point.y>=POOL_TOP?'pool':point.y>=POKER_TOP?'poker':point.y>=DINING_TOP?'dining':point.y>=TRAIN_TOP?'train':'airport';}
function areaTop(point){return point.y>=LOUNGE_TOP?LOUNGE_TOP:point.y>=POOL_TOP?POOL_TOP:point.y>=POKER_TOP?POKER_TOP:point.y>=DINING_TOP?DINING_TOP:point.y>=TRAIN_TOP?TRAIN_TOP:0;}
function combinedPositions(lessons){const positions=[...airportPositions(lessons.slice(0,30)),...trainPositions(),...diningPositions(),...pokerPositions(),...poolPositions(),...loungePositions()];return lessons.map((lesson,i)=>({...positions[i],id:lesson.id}));}
function combinedTerrain(nodes,lessons){return terrain(nodes.slice(0,30),lessons.slice(0,30))+trainTerrain()+diningTerrain()+pokerTerrain()+poolTerrain()+loungeTerrain()+[TRAIN_TOP,DINING_TOP,POKER_TOP,POOL_TOP,LOUNGE_TOP].map(y=>`<div class="business-theme-mist" style="top:${y-60}px" aria-hidden="true"></div>`).join('');}
function combinedMount(root,reduced,controls){
  const airport=mount(root,reduced),train=mountTrain(root,reduced),dining=mountDining(root,reduced),poker=mountPoker(root,reduced),pool=mountPool(root,reduced),lounge=mountLounge(root,reduced),events=new AbortController();
  ...
  tabs.innerHTML='<button type="button" data-business-area="airport" aria-pressed="true">機場貴賓室 · 01–30</button><button type="button" data-business-area="train" aria-pressed="false">臥鋪列車 · 31–60</button><button type="button" data-business-area="dining" aria-pressed="false">月夜餐車 · 61–90</button><button type="button" data-business-area="poker" aria-pressed="false">皇家牌桌 · 91–120</button><button type="button" data-business-area="pool" aria-pressed="false">桌球時光 · 121–150</button><button type="button" data-business-area="lounge" aria-pressed="false">爵士密語 · 151–180</button>';
  root.querySelector('.expression-map-tools')?.before(tabs);
  const note=document.createElement('div');note.className='train-reservation-note';note.hidden=true;root.querySelector('.expression-map-stage')?.append(note);
  root.querySelector('.expression-map-heading small').textContent='26 個課題已開放 · 180 個旅程平台';
  let currentArea='';
  function choose(area,point){
    if(area!==currentArea){currentArea=area;root.dataset.businessArea=area;tabs.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.businessArea===area)));note.hidden=area==='airport';note.textContent=area==='lounge'?'平台 151–180 · 課題準備中':area==='pool'?'平台 121–150 · 課題準備中':area==='poker'?'平台 91–120 · 課題準備中':area==='dining'?'平台 61–90 · 課題準備中':'平台 31–60 · 課題準備中';}
    if(point&&controls)controls.explore(point);
  }
  tabs.addEventListener('click',e=>{const b=e.target.closest('[data-business-area]');if(b){const area=b.dataset.businessArea;choose(area,area==='lounge'?loungePositions()[0]:area==='pool'?poolPositions()[0]:area==='poker'?pokerPositions()[0]:area==='dining'?diningPositions()[0]:area==='train'?trainPositions()[0]:airportPositions([{id:'first'}])[0]);}},{signal:events.signal});
  root.addEventListener('click',e=>{const b=e.target.closest('[data-train-platform],[data-dining-platform],[data-poker-platform],[data-pool-platform],[data-lounge-platform]');if(!b)return;e.stopImmediatePropagation();const order=Number(b.dataset.trainPlatform||b.dataset.diningPlatform||b.dataset.pokerPlatform||b.dataset.poolPlatform||b.dataset.loungePlatform),p=[...trainPositions(),...diningPositions(),...pokerPositions(),...poolPositions(),...loungePositions()].find(p=>p.order===order);root.querySelectorAll('[data-train-platform],[data-dining-platform],[data-poker-platform],[data-pool-platform],[data-lounge-platform]').forEach(n=>n.setAttribute('aria-pressed',String(n===b)));note.textContent=`平台 ${p.order} · 課題準備中`;controls?.explore(p,{walk:true});},{capture:true,signal:events.signal});
  return {draw(time){const position=controls?.getPosition();if(position)choose(areaFor(position));const scale=Number(root.dataset.scale)||1,top=(viewport?.scrollTop||0)/scale,bottom=top+(viewport?.clientHeight||7200)/scale;if(top<TRAIN_TOP)airport.draw(time);if(top<DINING_TOP&&bottom>TRAIN_TOP)train.draw(time);if(top<POKER_TOP&&bottom>DINING_TOP)dining.draw(time);if(top<POOL_TOP&&bottom>POKER_TOP)poker.draw(time);if(top<LOUNGE_TOP&&bottom>POOL_TOP)pool.draw(time);if(bottom>LOUNGE_TOP)lounge.draw(time);},destroy(){events.abort();airport.destroy();train.destroy();dining.destroy();poker.destroy();pool.destroy();lounge.destroy();tabs.remove();note.remove();}};
}
function businessFloor(point){if(point.y>=LOUNGE_TOP)return {x:clamp(point.x,285,1330),y:clamp(point.y,LOUNGE_TOP+425,LOUNGE_TOP+1080)};if(point.y>=POOL_TOP)return {x:clamp(point.x,220,1380),y:clamp(point.y,POOL_TOP+320,POOL_TOP+910)};if(point.y>=POKER_TOP)return {x:clamp(point.x,300,1370),y:clamp(point.y,POKER_TOP+435,POKER_TOP+1030)};if(point.y>=DINING_TOP)return {x:clamp(point.x,360,1390),y:clamp(point.y,DINING_TOP+575,DINING_TOP+1100)};if(point.y>=TRAIN_TOP)return {x:clamp(point.x,210,1400),y:clamp(point.y,TRAIN_TOP+545,TRAIN_TOP+1140)};return floorPoint(point);}
export const BUSINESS_AIRPORT=Object.freeze({id:'airport',title:'商務會話探索之旅',kicker:'BUSINESS SPEAKING · JOURNEYS',width:W,height:H*6,positions:combinedPositions,terrain:combinedTerrain,mount:combinedMount,fitOverview:true,overviewBounds:point=>({top:areaTop(point),height:H,key:areaFor(point)}),minimumZoom:.7,cameraScaleFloor:()=>.55,cameraTop:({point,scale,height,overview})=>{const start=areaTop(point);return overview||point.y-start<790?start*scale:point.y*scale-height*.55;},navigation:{path:(_from,to)=>[businessFloor(to)],step:(_from,to)=>businessFloor(to)}});
