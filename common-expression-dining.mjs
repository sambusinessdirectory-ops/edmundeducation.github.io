...
export function prepareDining(){
 return prepared ||= Promise.all([image(ART+'table-v1.jpg'),image(ART+'moonlit-lake-v1.jpg'),image(ART+'rose-v1.png'),image(ART+'tableware-v1.png')]).then(([background,landscape,rose,tableware])=>{
  ...
  return {background,tile,rose,roseRect:[l,t,r-l+1,b-t+1],parts,placeSetting:parts[0].toDataURL()};
...
export function mountDining(root,reduced){
 ...
 function render(seconds){if(!ready||dead)return;ctx.clearRect(0,0,1600,1200);
  ...
  for(const [x,y,rx,ry,a] of [[762,554,156,18,.22],[762,551,133,7,.25],[978,557,41,10,.17],[978,554,30,4,.22]]){ctx.save();ctx.translate(x,y);ctx.scale(rx,ry);const g=ctx.createRadialGradient(0,0,0,0,0,1);g.addColorStop(0,`rgba(49,30,15,${a})`);g.addColorStop(1,'rgba(49,30,15,0)');ctx.fillStyle=g;ctx.fillRect(-1,-1,2,2);ctx.restore();}
  ctx.save();ctx.filter='saturate(.75) brightness(.93)';ctx.drawImage(ready.parts[1],610,354,300,205);ctx.restore();ctx.drawImage(ready.parts[2],944,330,68,235);
