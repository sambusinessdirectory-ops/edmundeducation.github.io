...
import {MascotCharacters} from './speaking-mascot-characters.mjs?v=20260915-smooth1';
import {updateAttention,listenerNod} from './speaking-mascot-behaviour.mjs?v=20260915-smooth1';
...
export async function mountClassroom(root,candidates,onSelect,{seated=false}={}){
 ...
 let {yaw,pitch,distance}=CAMERA_START;
 ...
 }catch(error){observer.disconnect();sprites.dispose();environment?.dispose();renderer.dispose();renderer.forceContextLoss();root.textContent='3D could not load. Use the 2D mode.';throw error;}
 const zoom=delta=>{if(free){const direction=new THREE.Vector3();camera.getWorldDirection(direction);camera.position.addScaledVector(direction,-delta);constrainCamera(camera.position);}else{distance=THREE.MathUtils.clamp(camera.position.distanceTo(target)+delta,2.0,12);orbit();}};
 controls.onclick=e=>{const action=e.target.closest('[data-camera]')?.dataset.camera;if(action==='in')zoom(-.8);if(action==='out')zoom(.8);if(action==='reset'){free=false;({yaw,pitch,distance}=CAMERA_START);target.copy(CAMERA_START.target);orbit();}if(action==='free'){free=!free;if(free){const d=new THREE.Vector3();camera.getWorldDirection(d);yaw=Math.atan2(-d.x,-d.z);pitch=-Math.asin(d.y);look();}else{target.copy(camera.position).addScaledVector(camera.getWorldDirection(new THREE.Vector3()),5);constrainTarget(target);distance=Math.max(.2,camera.position.distanceTo(target));yaw=Math.atan2(camera.position.x-target.x,camera.position.z-target.z);pitch=Math.asin((camera.position.y-target.y)/distance);orbit();}}controls.querySelector('[data-camera="free"]').setAttribute('aria-pressed',String(free));controls.querySelector('[data-camera-move]').hidden=!free;};
