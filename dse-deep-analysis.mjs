...
export function createDeepAnalysisReader({ verifySourceAdmin = async () => false } = {}) {
  ...
  let request = 0, sourceAdmin = false;
  ...
  dialog.addEventListener('click', event=>{
    ...
    if(button.dataset.deepMode) { if(button.dataset.deepMode==='original' && !sourceAdmin) return; mode=button.dataset.deepMode; render(); return focusContent(); }
  ...
  dialog.addEventListener('toggle',event=>{
    if(sourceAdmin && event.target.matches('.deep-original') && event.target.open) { const template=event.target.querySelector('template'); if(template) template.outerHTML=original(data.pages[Number(template.dataset.deepImage)-1]); }
  },true);
  async function open(nextArticle, number, nextUserId, nextAnswers, trigger=document.activeElement) {
    ...
    selected=0;mode='guided';query='';category='';supplement=false; progress={}; sourceAdmin=false;
    ...
    try {
      const verified = await Promise.resolve().then(verifySourceAdmin).catch(() => false);
      if(ticket!==request || !dialog.open) return;
      sourceAdmin = verified === true;
