-- Edmund Education Proverb System: durable attempts, bookmarks, and admin auth.
--
-- Flashcard remains the only student credential store. This migration creates
-- no Proverb System student or password table. Apply
...
begin
  if p_lesson_id not in ('proverb-01', 'proverb-02', 'proverb-03')
    or p_result is null
...
create or replace function public._proverb_system_bookmark_payload_valid(p_bookmarks jsonb)
returns boolean
language plpgsql
immutable
set search_path = ''
...
begin
  if p_bookmarks is null
    ...
    or jsonb_array_length(p_bookmarks) > 153
  ...
  loop
    if jsonb_typeof(v_item) <> 'object'
      ...
      or coalesce(v_item ->> 'lessonId', '') not in ('proverb-01', 'proverb-02', 'proverb-03')
...
create table if not exists public.proverb_system_admin_accounts (
...
create unique index if not exists proverb_system_admin_name_lower_idx
  on public.proverb_system_admin_accounts (lower(name));

-- Raw bearer tokens are returned once. Supabase persists only SHA-256 digests.
create table if not exists public.proverb_system_admin_sessions (
  token_hash bytea primary key,
  admin_id uuid not null
    references public.proverb_system_admin_accounts(id) on delete cascade,
  created_at timestamptz not null default now(),
...
create index if not exists proverb_system_admin_sessions_expires_idx
  on public.proverb_system_admin_sessions (expires_at);

create table if not exists public.proverb_system_attempts (
  id uuid primary key,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint proverb_system_attempts_lesson_id_check
    check (lesson_id in ('proverb-01', 'proverb-02', 'proverb-03')),
  ...
  check (public._proverb_system_result_valid(lesson_id, result)),
  check (
...
create index if not exists proverb_system_attempts_student_history_idx
  on public.proverb_system_attempts (student_id, updated_at desc, id desc);

create index if not exists proverb_system_attempts_student_lesson_idx
  on public.proverb_system_attempts (student_id, lesson_id, status, updated_at desc);

create table if not exists public.proverb_system_bookmarks (
  student_id uuid not null
    references public.flashcard_students(id) on delete cascade,
  lesson_id text not null,
  ...
  constraint proverb_system_bookmarks_lesson_id_check
    check (lesson_id in ('proverb-01', 'proverb-02', 'proverb-03')),
  constraint proverb_system_bookmarks_question_id_check
    check (
...
-- `proverb_system_bookmarks_check`; drop that legacy name as well.
...
create index if not exists proverb_system_bookmarks_student_created_idx
  on public.proverb_system_bookmarks (student_id, created_at desc, lesson_id, question_id);

alter table public.proverb_system_admin_accounts enable row level security;
alter table public.proverb_system_admin_sessions enable row level security;
alter table public.proverb_system_attempts enable row level security;
alter table public.proverb_system_bookmarks enable row level security;
...
revoke all on table public.proverb_system_admin_accounts
  from public, anon, authenticated, service_role;
revoke all on table public.proverb_system_admin_sessions
  from public, anon, authenticated, service_role;
revoke all on table public.proverb_system_attempts
  from public, anon, authenticated, service_role;
revoke all on table public.proverb_system_bookmarks
  from public, anon, authenticated, service_role;

create or replace function public._proverb_system_revoke_admin_sessions()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  then
    delete from public.proverb_system_admin_sessions session_row
...
drop trigger if exists proverb_system_admin_security_change
  on public.proverb_system_admin_accounts;
create trigger proverb_system_admin_security_change
after update of password_hash, is_active on public.proverb_system_admin_accounts
for each row execute function public._proverb_system_revoke_admin_sessions();

create or replace function public._proverb_system_admin_id(p_admin_token uuid)
returns uuid
language sql
stable
security definer
...
as $$
  ...
  from public.proverb_system_admin_sessions session_row
  join public.proverb_system_admin_accounts account
...
-- active Proverb System admin sessions.
create or replace function public.proverb_system_provision_admin(
  p_name text,
  p_bcrypt_hash text
)
...
begin
  ...
  select account.id
  into v_admin_id
  from public.proverb_system_admin_accounts account
  where lower(account.name) = lower(v_name)
  limit 1
  for update;

  if v_admin_id is null then
    insert into public.proverb_system_admin_accounts (
      name,
      password_hash,
      is_active
    )
    ...
    update public.proverb_system_admin_accounts account
    set name = v_name,
...
create or replace function public.proverb_system_admin_login(
  p_name text,
  p_password text
)
returns table (
...
declare
  ...
  v_admin public.proverb_system_admin_accounts%rowtype;
...
begin
  ...
  insert into public.proverb_system_admin_sessions (
    token_hash,
    admin_id,
    created_at,
    expires_at
...
create or replace function public.proverb_system_admin_me(p_admin_token uuid)
...
create or replace function public.proverb_system_admin_logout(p_admin_token uuid)
returns boolean
language plpgsql
security definer
set search_path = ''
...
create or replace function public.proverb_system_student_profile(p_token uuid)
returns table (
  id uuid,
  name text,
...
as $$
declare
  v_existing public.proverb_system_attempts%rowtype;
  v_now timestamptz := clock_timestamp();
...
begin
  ...
  if p_id is null
    or p_lesson_id not in ('proverb-01', 'proverb-02', 'proverb-03')
    ...
    or not public._proverb_system_result_valid(p_lesson_id, p_result)
  ...
  then
    raise exception 'Invalid Proverb System attempt' using errcode = '22023';
  end if;
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'proverb-system-student:' || p_student_id::text,
      0
    )
  );
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'proverb-system-attempt:' || p_id::text,
      0
    )
  );
  ...
  select attempt.*
  into v_existing
  from public.proverb_system_attempts attempt
  ...
  if found then
    ...
    if v_existing.status <> 'completed' then
      ...
      update public.proverb_system_attempts attempt
      set status = p_status,
    ...
    insert into public.proverb_system_attempts (
      id,
      student_id,
      lesson_id,
      lesson_version,
...
create or replace function public.proverb_system_list_attempts(
  p_student_id uuid,
  p_limit integer,
  p_offset integer
)
...
create or replace function public.proverb_system_replace_bookmarks(
  p_student_id uuid,
  p_bookmarks jsonb
)
returns table (
...
begin
  ...
  if not public._proverb_system_bookmark_payload_valid(p_bookmarks) then
    raise exception 'Invalid Proverb System bookmarks' using errcode = '22023';
  end if;

  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'proverb-system-bookmarks:' || p_student_id::text,
      0
    )
  );

  insert into public.proverb_system_bookmarks as bookmark (
    student_id,
    lesson_id,
    question_id,
    include_answer,
  ...
  on conflict on constraint proverb_system_bookmarks_pkey do update
  set include_answer = excluded.include_answer,
      updated_at = now();

  delete from public.proverb_system_bookmarks bookmark
  ...
  select
    ...
    bookmark.created_at
  from public.proverb_system_bookmarks bookmark
  where bookmark.student_id = p_student_id
  order by bookmark.created_at desc, bookmark.lesson_id, bookmark.question_id;
end;
$$;

create or replace function public.proverb_system_list_bookmarks(p_student_id uuid)
returns table (
  lesson_id text,
  question_id text,
  include_answer boolean,
...
as $$
begin
  if p_offset not between 0 and 153
...
as $$
begin
  if public._proverb_system_admin_id(p_admin_token) is null then
    return;
  end if;
...
create or replace function public.proverb_system_admin_student_profile(
  p_admin_token uuid,
  p_student_id uuid
)
...
as $$
  ...
  from public.flashcard_students student
  where public._proverb_system_admin_id(p_admin_token) is not null
    and student.id = p_student_id
    and student.deleted_at is null
  limit 1;
$$;

create or replace function public.proverb_system_admin_list_attempts(
  p_admin_token uuid,
  p_student_id uuid,
  p_limit integer
)
...
create or replace function public.proverb_system_admin_list_bookmarks(
  p_admin_token uuid,
  p_student_id uuid
)
returns table (
...
revoke all on function public._proverb_system_bookmark_payload_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function public._proverb_system_revoke_admin_sessions()
  from public, anon, authenticated, service_role;
revoke all on function public._proverb_system_admin_id(uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.proverb_system_provision_admin(text, text)
  from public, anon, authenticated, service_role;
revoke all on function public.proverb_system_admin_login(text, text)
  from public, anon, authenticated;
revoke all on function public.proverb_system_admin_me(uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_admin_logout(uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_student_profile(uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_upsert_attempt(
  ...
  from public, anon, authenticated;
revoke all on function public.proverb_system_get_attempt(uuid, uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_list_attempts(uuid, integer, integer)
  from public, anon, authenticated;
revoke all on function public.proverb_system_replace_bookmarks(uuid, jsonb)
  from public, anon, authenticated;
revoke all on function public.proverb_system_list_bookmarks(uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_list_bookmarks_page(uuid, integer, integer)
  from public, anon, authenticated;
revoke all on function public.proverb_system_admin_list_students(uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_admin_student_profile(uuid, uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_admin_list_attempts(uuid, uuid, integer)
  from public, anon, authenticated;
revoke all on function public.proverb_system_admin_list_bookmarks(uuid, uuid)
  from public, anon, authenticated;
revoke all on function public.proverb_system_admin_list_bookmarks_page(
  uuid, uuid, integer, integer
)
  from public, anon, authenticated;

grant execute on function public.proverb_system_admin_login(text, text)
  to service_role;
grant execute on function public.proverb_system_admin_me(uuid)
  to service_role;
grant execute on function public.proverb_system_admin_logout(uuid)
  to service_role;
grant execute on function public.proverb_system_student_profile(uuid)
  to service_role;
grant execute on function public.proverb_system_upsert_attempt(
  ...
  to service_role;
grant execute on function public.proverb_system_get_attempt(uuid, uuid)
  to service_role;
grant execute on function public.proverb_system_list_attempts(uuid, integer, integer)
  to service_role;
grant execute on function public.proverb_system_replace_bookmarks(uuid, jsonb)
  to service_role;
grant execute on function public.proverb_system_list_bookmarks(uuid)
  to service_role;
grant execute on function public.proverb_system_list_bookmarks_page(uuid, integer, integer)
  to service_role;
grant execute on function public.proverb_system_admin_list_students(uuid)
  to service_role;
grant execute on function public.proverb_system_admin_student_profile(uuid, uuid)
  to service_role;
grant execute on function public.proverb_system_admin_list_attempts(uuid, uuid, integer)
  to service_role;
grant execute on function public.proverb_system_admin_list_bookmarks(uuid, uuid)
  to service_role;
grant execute on function public.proverb_system_admin_list_bookmarks_page(
  uuid, uuid, integer, integer
)
  to service_role;
