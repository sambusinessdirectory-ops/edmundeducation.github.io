...
export async function previewEmail({content,recipients,imageSource,signatureLink,attachments,sender,action='確認發送',checkSpelling=checkEmailSpelling,onStage=()=>{}}) {
  ...
  onStage('preview_opened');
  return new Promise(resolve=>{
    let checked=false,spellcheck='not_checked',busy=false,finished=false;
    const finish=value=>{if(finished)return;finished=true;dialog.close();dialog.remove();onStage(value?'preview_confirmed':'preview_cancelled');resolve(value);};
    dialog.querySelector('[data-close]').onclick=()=>finish(null);
    dialog.querySelector('[data-edit]').onclick=()=>finish(null);
    dialog.addEventListener('cancel',event=>{event.preventDefault();finish(null);});
    const button=dialog.querySelector('[data-next]');
    button.type='button';
    button.onclick=()=>{if(checked && !busy) finish({spellcheck});};
    const check=async()=>{
      if(busy) return;
      busy=true;button.disabled=true;button.textContent='正在檢查拼字…';
      dialog.querySelector('[data-progress]').textContent='尚未發送。正在此裝置檢查英文拼字；完成後按下方確認按鈕才會提交。';
      let timer;
      try {
        ...
        dialog.querySelector('[data-progress]').textContent=issues.length?'尚未發送。請檢查以上提醒，然後確認。':'尚未發送。未發現明顯英文拼字問題；請按下方按鈕作最後確認。';
      ...
      } finally {clearTimeout(timer);checked=true;busy=false;button.disabled=false;if(!finished)onStage('preview_check_finished');}
    };
    // Check while the admin reads the preview. There is exactly one final
    // confirmation, never a second button masquerading as a send operation.
    void check();
  });
}
