-- Edmund Education Idiom System: durable attempts, bookmarks, and admin auth.
--
-- Flashcard remains the only student credential store. This migration creates
-- no Idiom System student or password table. Apply
...
create or replace function public._idiom_system_result_valid(
  p_lesson_id text,
  p_result jsonb
)
returns boolean
...
begin
  if p_lesson_id !~ '^idiom-(0[1-9]|[1-9][0-9]|1[0-2][0-9]|13[0-8])$'
    or p_result is null
...
create or replace function public._idiom_system_bookmark_payload_valid(p_bookmarks jsonb)
returns boolean
language plpgsql
immutable
set search_path = ''
...
begin
  if p_bookmarks is null
    ...
    or jsonb_array_length(p_bookmarks) > 7038
  ...
  loop
    if jsonb_typeof(v_item) <> 'object'
      ...
      or coalesce(v_item ->> 'lessonId', '') !~ '^idiom-(0[1-9]|[1-9][0-9]|1[0-2][0-9]|13[0-8])$'
...
create table if not exists public.idiom_system_admin_accounts (
...
create unique index if not exists idiom_system_admin_name_lower_idx
  on public.idiom_system_admin_accounts (lower(name));

-- Raw bearer tokens are returned once. Supabase persists only SHA-256 digests.
create table if not exists public.idiom_system_admin_sessions (
  token_hash bytea primary key,
  admin_id uuid not null
    references public.idiom_system_admin_accounts(id) on delete cascade,
  created_at timestamptz not null default now(),
...
create index if not exists idiom_system_admin_sessions_expires_idx
  on public.idiom_system_admin_sessions (expires_at);

create table if not exists public.idiom_system_attempts (
  id uuid primary key,
  ...
  constraint idiom_system_attempts_lesson_id_check
    check (lesson_id ~ '^idiom-(0[1-9]|[1-9][0-9]|1[0-2][0-9]|13[0-8])$'),
  ...
  check (public._idiom_system_result_valid(lesson_id, result)),
  check (
...
create index if not exists idiom_system_attempts_student_history_idx
  on public.idiom_system_attempts (student_id, updated_at desc, id desc);

create index if not exists idiom_system_attempts_student_lesson_idx
  on public.idiom_system_attempts (student_id, lesson_id, status, updated_at desc);

create table if not exists public.idiom_system_bookmarks (
  student_id uuid not null
    references public.flashcard_students(id) on delete cascade,
  lesson_id text not null,
...
-- `idiom_system_bookmarks_check`; drop that legacy name as well.
...
alter table public.idiom_system_admin_accounts enable row level security;
alter table public.idiom_system_admin_sessions enable row level security;
alter table public.idiom_system_attempts enable row level security;
alter table public.idiom_system_bookmarks enable row level security;
...
revoke all on table public.idiom_system_admin_accounts
  from public, anon, authenticated, service_role;
revoke all on table public.idiom_system_admin_sessions
  from public, anon, authenticated, service_role;
revoke all on table public.idiom_system_attempts
  from public, anon, authenticated, service_role;
revoke all on table public.idiom_system_bookmarks
  from public, anon, authenticated, service_role;

create or replace function public._idiom_system_revoke_admin_sessions()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  then
    delete from public.idiom_system_admin_sessions session_row
...
drop trigger if exists idiom_system_admin_security_change
  on public.idiom_system_admin_accounts;
create trigger idiom_system_admin_security_change
after update of password_hash, is_active on public.idiom_system_admin_accounts
for each row execute function public._idiom_system_revoke_admin_sessions();

create or replace function public._idiom_system_admin_id(p_admin_token uuid)
returns uuid
language sql
stable
security definer
...
as $$
  ...
  from public.idiom_system_admin_sessions session_row
  join public.idiom_system_admin_accounts account
...
-- active Idiom System admin sessions.
create or replace function public.idiom_system_provision_admin(
  p_name text,
  p_bcrypt_hash text
)
...
begin
  ...
  select account.id
  into v_admin_id
  from public.idiom_system_admin_accounts account
  where lower(account.name) = lower(v_name)
  limit 1
  for update;

  if v_admin_id is null then
    insert into public.idiom_system_admin_accounts (
      name,
      password_hash,
      is_active
    )
    ...
    update public.idiom_system_admin_accounts account
    set name = v_name,
...
create or replace function public.idiom_system_admin_login(
  p_name text,
  p_password text
)
returns table (
...
declare
  ...
  v_admin public.idiom_system_admin_accounts%rowtype;
...
begin
  ...
  insert into public.idiom_system_admin_sessions (
    token_hash,
    admin_id,
    created_at,
    expires_at
...
create or replace function public.idiom_system_admin_me(p_admin_token uuid)
...
create or replace function public.idiom_system_admin_logout(p_admin_token uuid)
returns boolean
language plpgsql
security definer
set search_path = ''
...
create or replace function public.idiom_system_student_profile(p_token uuid)
returns table (
  id uuid,
  name text,
...
create or replace function public.idiom_system_upsert_attempt(
  p_id uuid,
  p_student_id uuid,
  p_lesson_id text,
...
as $$
declare
  v_existing public.idiom_system_attempts%rowtype;
  v_now timestamptz := clock_timestamp();
...
begin
  ...
  if p_id is null
    or p_lesson_id !~ '^idiom-(0[1-9]|[1-9][0-9]|1[0-2][0-9]|13[0-8])$'
    ...
    or not public._idiom_system_result_valid(p_lesson_id, p_result)
  ...
  then
    raise exception 'Invalid Idiom System attempt' using errcode = '22023';
  end if;
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'idiom-system-student:' || p_student_id::text,
      0
    )
  );
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'idiom-system-attempt:' || p_id::text,
      0
    )
  );
  ...
  select attempt.*
  into v_existing
  from public.idiom_system_attempts attempt
  ...
  if found then
    ...
    if v_existing.status <> 'completed' then
      ...
      update public.idiom_system_attempts attempt
      set status = p_status,
    ...
    insert into public.idiom_system_attempts (
      id,
      student_id,
      lesson_id,
      lesson_version,
...
create or replace function public.idiom_system_get_attempt(
  p_student_id uuid,
  p_id uuid
)
returns table (
...
create or replace function public.idiom_system_list_attempts(
  p_student_id uuid,
  p_limit integer,
  p_offset integer
)
...
create or replace function public.idiom_system_replace_bookmarks(
  p_student_id uuid,
  p_bookmarks jsonb
)
returns table (
...
begin
  ...
  if not public._idiom_system_bookmark_payload_valid(p_bookmarks) then
    raise exception 'Invalid Idiom System bookmarks' using errcode = '22023';
  end if;

  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'idiom-system-bookmarks:' || p_student_id::text,
      0
    )
  );

  insert into public.idiom_system_bookmarks as bookmark (
    student_id,
    lesson_id,
    question_id,
    include_answer,
  ...
  on conflict on constraint idiom_system_bookmarks_pkey do update
  set include_answer = excluded.include_answer,
      updated_at = now();

  delete from public.idiom_system_bookmarks bookmark
  ...
  select
    ...
    bookmark.created_at
  from public.idiom_system_bookmarks bookmark
  where bookmark.student_id = p_student_id
  order by bookmark.created_at desc, bookmark.lesson_id, bookmark.question_id;
end;
$$;

create or replace function public.idiom_system_list_bookmarks(p_student_id uuid)
returns table (
  lesson_id text,
  question_id text,
  include_answer boolean,
...
create or replace function public.idiom_system_list_bookmarks_page(
  p_student_id uuid,
  p_offset integer,
  p_limit integer
)
...
as $$
begin
  if p_offset not between 0 and 7038
...
create or replace function public.idiom_system_admin_list_students(
  p_admin_token uuid
)
returns table (
  id uuid,
...
create or replace function public.idiom_system_admin_student_profile(
  p_admin_token uuid,
  p_student_id uuid
)
...
as $$
  ...
  from public.flashcard_students student
  where public._idiom_system_admin_id(p_admin_token) is not null
    and student.id = p_student_id
    and student.deleted_at is null
  limit 1;
$$;

create or replace function public.idiom_system_admin_list_attempts(
  p_admin_token uuid,
  p_student_id uuid,
  p_limit integer
)
...
create or replace function public.idiom_system_admin_list_bookmarks(
  p_admin_token uuid,
  p_student_id uuid
)
returns table (
...
create or replace function public.idiom_system_admin_list_bookmarks_page(
  p_admin_token uuid,
  p_student_id uuid,
  p_offset integer,
  p_limit integer
...
revoke all on function public._idiom_system_bookmark_payload_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function public._idiom_system_revoke_admin_sessions()
  from public, anon, authenticated, service_role;
revoke all on function public._idiom_system_admin_id(uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.idiom_system_provision_admin(text, text)
  from public, anon, authenticated, service_role;
revoke all on function public.idiom_system_admin_login(text, text)
  from public, anon, authenticated;
revoke all on function public.idiom_system_admin_me(uuid)
  from public, anon, authenticated;
revoke all on function public.idiom_system_admin_logout(uuid)
  from public, anon, authenticated;
revoke all on function public.idiom_system_student_profile(uuid)
  from public, anon, authenticated;
revoke all on function public.idiom_system_upsert_attempt(
  ...
  from public, anon, authenticated;
revoke all on function public.idiom_system_get_attempt(uuid, uuid)
  from public, anon, authenticated;
revoke all on function public.idiom_system_list_attempts(uuid, integer, integer)
  from public, anon, authenticated;
revoke all on function public.idiom_system_replace_bookmarks(uuid, jsonb)
  from public, anon, authenticated;
revoke all on function public.idiom_system_list_bookmarks(uuid)
  from public, anon, authenticated;
revoke all on function public.idiom_system_list_bookmarks_page(uuid, integer, integer)
  from public, anon, authenticated;
revoke all on function public.idiom_system_admin_list_students(uuid)
  from public, anon, authenticated;
revoke all on function public.idiom_system_admin_student_profile(uuid, uuid)
  from public, anon, authenticated;
revoke all on function public.idiom_system_admin_list_attempts(uuid, uuid, integer)
  from public, anon, authenticated;
revoke all on function public.idiom_system_admin_list_bookmarks(uuid, uuid)
  from public, anon, authenticated;
...
grant execute on function public.idiom_system_admin_login(text, text)
  to service_role;
grant execute on function public.idiom_system_admin_me(uuid)
  to service_role;
grant execute on function public.idiom_system_admin_logout(uuid)
  to service_role;
grant execute on function public.idiom_system_student_profile(uuid)
  to service_role;
grant execute on function public.idiom_system_upsert_attempt(
  ...
  to service_role;
grant execute on function public.idiom_system_get_attempt(uuid, uuid)
  to service_role;
grant execute on function public.idiom_system_list_attempts(uuid, integer, integer)
  to service_role;
grant execute on function public.idiom_system_replace_bookmarks(uuid, jsonb)
  to service_role;
grant execute on function public.idiom_system_list_bookmarks(uuid)
  to service_role;
grant execute on function public.idiom_system_list_bookmarks_page(uuid, integer, integer)
  to service_role;
grant execute on function public.idiom_system_admin_list_students(uuid)
  to service_role;
grant execute on function public.idiom_system_admin_student_profile(uuid, uuid)
  to service_role;
grant execute on function public.idiom_system_admin_list_attempts(uuid, uuid, integer)
  to service_role;
grant execute on function public.idiom_system_admin_list_bookmarks(uuid, uuid)
  to service_role;
