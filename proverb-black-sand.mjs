...
export const BLACK_SEA='M0 320H1160L1100 365L1035 410L965 450L870 485L760 519L645 549L510 575L440 580L445 540L425 505L350 475L240 453L105 430L0 413Z';
...
// Classify the painted water inside conservative shore boundaries, then inset the
// mask farther than the maximum texture displacement. Rocks cannot enter samples.
function waterLayer(background,path,box){
 const [x,y,w,h]=box,mask=document.createElement('canvas');mask.width=w;mask.height=h;
 const c=mask.getContext('2d');c.translate(-x,-y);c.fill(new Path2D(path));c.setTransform(1,0,0,1,0,0);
 const shape=c.getImageData(0,0,w,h),pixels=background.getContext('2d').getImageData(x,y,w,h),distance=new Float32Array(w*h);
 for(let j=0;j<h;j++)for(let i=0;i<w;i++){const n=j*w+i,k=n*4,[r,g,b]=pixels.data.subarray(k,k+3);
  const water=(g>r*1.035&&b>r*.96&&g>65)||(Math.min(r,g,b)>175&&Math.max(r,g,b)-Math.min(r,g,b)<36);
  distance[n]=shape.data[k+3]&&water&&i&&j&&i<w-1&&j<h-1?10000:0;
 }
 for(let j=1;j<h;j++)for(let i=1;i<w;i++){const n=j*w+i;distance[n]=Math.min(distance[n],distance[n-1]+1,distance[n-w]+1);}
 for(let j=h-2;j>=0;j--)for(let i=w-2;i>=0;i--){const n=j*w+i;distance[n]=Math.min(distance[n],distance[n+1]+1,distance[n+w]+1);}
 for(let n=0;n<distance.length;n++)shape.data[n*4+3]=Math.max(0,Math.min(1,(distance[n]-(w<400?3:6))/(w<400?4:8)))*255;
 c.putImageData(shape,0,0);const texture=document.createElement('canvas');texture.width=w;texture.height=h;
 return {box,mask,texture};
}
...
function mount(root,reduced,controls){const canvas=root.querySelector('.black-motion'),ctx=canvas.getContext('2d'),events=new AbortController();let ready,waters,dead=false,last=0,t=0,painted=-1;
 ...
 prepareBlackSand().then(a=>{if(!dead){ready=a;waters=[waterLayer(a.background,BLACK_SEA,[0,310,1480,315]),waterLayer(a.background,POOLS[0],[0,845,312,140]),waterLayer(a.background,POOLS[1],[1435,1268,165,183])];render(0);}});
 // Animate only original painted water. No synthetic foam lines or ripple rings.
 function water(layer,time,pool=false){
  const {box:[bx,by,bw,bh],mask,texture}=layer,c=texture.getContext('2d');c.clearRect(0,0,bw,bh);
  for(let y=0;y<bh;y+=2){const dx=Math.sin((by+y)*.035-time*.95)*(pool?1.7:3),dy=Math.sin((by+y)*.025-time*1.2)*(pool?1:2);
   c.drawImage(ready.background,bx+dx,by+y+dy,bw,2,0,y,bw,2);
  }
  c.globalCompositeOperation='destination-in';c.drawImage(mask,0,0);c.globalCompositeOperation='source-over';
  ctx.save();ctx.globalAlpha=.88;ctx.drawImage(texture,bx,by);ctx.restore();
 }
 ...
 function render(time){if(!ready||dead)return;ctx.clearRect(0,0,1600,1950);waters.forEach((layer,i)=>water(layer,time,i>0));
