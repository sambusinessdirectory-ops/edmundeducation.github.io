...
import { cosmeticAtlas, restoreCosmetics, subscribeCosmetics } from './eddy-cosmetics.mjs?v=20260916-girls-individual1';
...
export function createExpressionMap({ root, toggle, grid, lessons, getCompleted, openLesson, systemKey = 'speaking', theme = null }) {
  ...
  const openCloset = async () => {
    ...
    try {
      const { openCompanionCloset } = await import('./common-expression-closet-3d.mjs?v=20260916-girls-individual1');
  ...
  function build() {
    ...
    root.querySelector('.expression-map-characters legend').after(closetButton);
