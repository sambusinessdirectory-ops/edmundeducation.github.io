...
import { cosmeticAtlas, restoreCosmetics, subscribeCosmetics } from './eddy-cosmetics.mjs?v=20260923-admin-cosmetic-preview1';
...
export function levelPositions(lessons, { startY = 200, rowGap = 350, rowYs = [], startX = 200, columnGap = 210 } = {}) {
  return lessons.map((lesson, i) => {
    const row = Math.floor(i / 7), col = row % 2 ? 6 - i % 7 : i % 7;
    return { id: lesson.id, x: startX + col * columnGap, y: (rowYs[row] ?? startY + row * rowGap) + Math.sin(col * 1.15) * 18 };
  });
}
...
export function createExpressionMap({ root, toggle, grid, lessons, getCompleted, openLesson, systemKey = 'speaking', theme = null }) {
  ...
  const openCloset = async () => {
    ...
    try {
      const { openCompanionCloset } = await import('./common-expression-closet-3d.mjs?v=20260923-admin-cosmetic-preview1');
      if (request !== closetRequest) return;
      closetHandle?.close();
      closetHandle = openCompanionCloset({ character });
    } catch (error) {
      console.error('Could not open the 3D closet', error);
      if (status) {
        status.hidden = false;
        status.textContent = '3D 衣櫥未能開啟，請重新整理後再試。';
      }
  ...
  function updateSelection() {
    ...
    root.querySelectorAll('[data-character]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.character===character)));const coin=root.querySelector('[data-eddy-map-coins]'),wallet=document.querySelector('[data-edmund-coin-wallet] strong');if(coin)coin.textContent=wallet?.textContent||'—';
  ...
  function build() {
    ...
    root.querySelector('.expression-map-characters legend').after(closetButton);window.addEventListener('edmund-coin-wallet-refresh',updateSelection);window.addEventListener('edmund-student-session-change',updateSelection);const walletActions=document.querySelector('.edmund-system-header__actions');if(walletActions&&typeof MutationObserver!=='undefined'){const walletObserver=new MutationObserver(updateSelection);walletObserver.observe(walletActions,{childList:true,subtree:true,characterData:true});}
  ...
  return {
    ...
    reset() { active=false; owner=''; mode=false; stopAnimation();closeCloset();clearTimeout(statusTimer);grid.hidden=false;root.hidden=true; },
    destroy() { stopAnimation();closeCloset();clearTimeout(statusTimer);events.abort();sceneAnimation?.destroy();resizeObserver?.disconnect();intersectObserver?.disconnect();root.replaceChildren(); }
  };
}
