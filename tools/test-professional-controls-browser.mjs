...
const states={},messages=[];let photo=null,photoRevision=0;const photoRequests=[];let apiCalls=0;
async function setup(id='alice',admin=false){
 ...
 await context.route('**/rest/v1/**',async route=>{
  ...
  if(name==='special_flash_course_photo'){photoRequests.push(args);if(!args.p_course)payload={courses:id==='outside'?[]:[{id:'qa-course',title:'Our course',can_edit:id==='editor'}]};else{if(args.p_action==='save'||args.p_action==='remove'){assert.equal(id,'editor');assert.equal(args.p_revision,photoRevision);photo=args.p_action==='save'?args.p_image:null;photoRevision++;}payload={course_id:'qa-course',revision:photoRevision,can_edit:id==='editor',changed:args.p_revision!==photoRevision,image:args.p_revision!==photoRevision?photo:null};}}
...
const teacher=await setup('editor');await teacher.page.setViewportSize({width:1280,height:900});await teacher.page.goto(base);await teacher.page.locator('[data-photo-upload]').waitFor();
const png=await teacher.page.evaluate(()=>{const c=document.createElement('canvas');c.width=800;c.height=500;const x=c.getContext('2d');x.fillStyle='#278b6a';x.fillRect(0,0,800,500);x.fillStyle='#fff';x.font='50px sans-serif';x.fillText('Course photo',70,260);return c.toDataURL('image/png').split(',')[1];});
await teacher.page.locator('[data-photo-file]').setInputFiles({name:'course.png',mimeType:'image/png',buffer:Buffer.from(png,'base64')});await teacher.page.getByText('已更新，同課程的學生可看到。').waitFor();assert.ok(photo.startsWith('data:image/jpeg;base64,'));assert.ok(photo.length<500000);assert.equal(photoRevision,1);
...
await teacher.page.locator('[data-photo-file]').setInputFiles({name:'replacement.png',mimeType:'image/png',buffer:Buffer.from(png,'base64')});await teacher.page.getByText('已更新，同課程的學生可看到。').waitFor();assert.equal(photoRevision,2);await peer.page.evaluate(()=>window.dispatchEvent(new Event('focus')));await peer.page.waitForTimeout(200);assert.ok(photoRequests.some(r=>r.p_revision===1&&r.p_action===undefined),'peer checks newer revision automatically');
await teacher.page.setViewportSize({width:390,height:844});await teacher.page.locator('.course-photo-slot').scrollIntoViewIfNeeded();assert.ok(await teacher.page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await teacher.page.screenshot({path:output+'/photo-mobile.png'});
await teacher.page.locator('[data-photo-remove]').click();await teacher.page.getByText('已更新，同課程的學生可看到。').waitFor();assert.equal(photo,null);await peer.page.evaluate(()=>window.dispatchEvent(new Event('focus')));await peer.page.locator('.course-photo-slot').waitFor({state:'hidden'});
