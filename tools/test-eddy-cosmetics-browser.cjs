...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const origin='http://127.0.0.1:'+server.address().port,browser=await chromium.launch({headless:true});try{
 ...
 await page.evaluate(async()=>{
  ...
  const {mountClosetInventory}=await import('/eddy-closet-inventory.mjs?v=20260923-pink-rain-jacket1');mountClosetInventory(document.querySelector('#inventory'),new AbortController().signal);
 });
 ...
 await page.reload();await page.evaluate(async()=>{window.cosmetics=await import('/eddy-cosmetics.mjs?v=20260923-pink-rain-jacket1');await cosmetics.restoreCosmetics();});assert.deepEqual(await page.evaluate(()=>cosmetics.cosmeticsState().equipped),saved.equipped);
