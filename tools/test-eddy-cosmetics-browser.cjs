...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const origin='http://127.0.0.1:'+server.address().port,browser=await chromium.launch({headless:true});try{
 ...
 await page.evaluate(async()=>{
  window.cosmetics=await import('/eddy-cosmetics.mjs?v=20260915-closet2');await cosmetics.restoreCosmetics();
  window.base=new Image();base.src='/assets/speaking-system/mascots/v4/eddy-standing.png';await base.decode();
  const {mountClosetInventory}=await import('/eddy-closet-inventory.mjs?v=20260915-closet2');mountClosetInventory(document.querySelector('#inventory'),new AbortController().signal);
 });
 ...
 await page.reload();await page.evaluate(async()=>{window.cosmetics=await import('/eddy-cosmetics.mjs?v=20260915-closet2');await cosmetics.restoreCosmetics();});assert.deepEqual(await page.evaluate(()=>cosmetics.cosmeticsState().equipped),saved.equipped);
 ...
 await page.evaluate(async()=>{
  ...
  const {MascotCharacters}=await import('/speaking-mascot-characters.mjs?v=20260915-closet2');
