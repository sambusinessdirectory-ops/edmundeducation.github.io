...
const sharedNavCssRelease = "20260924-wallet1";
const sharedNavJsRelease = "20260924-wallet1";
...
test("every system portal loads one consistent shared navigation CSS and JS release", () => {
  ...
  for (const system of Array.from(api.systems)) {
    ...
    assert.doesNotMatch(html, /shared-system-nav\.css\?v=(?!20260924-wallet1)|shared-system-nav\.js\?v=(?!20260924-wallet1)/, `${system.href} must not retain a stale shared navigation release`);
  }
...
test("every system header receives one shared account-aware Pomodoro controller", () => {
  ...
  assert.match(scriptSource, /switcher\.insertAdjacentElement\("afterend", button\)/,
    "the shared header order must be switcher + Homework, then Pomodoro");
  assert.doesNotMatch(cssSource, /\.edmund-pomodoro-header-button\s*\{[^}]*order:\s*-1/s,
    "Pomodoro must not visually jump ahead of the Edmund switcher and Homework shortcut");
