...
const sharedNavRelease = "20260908-refine7";
...
test("every system portal loads one consistent shared navigation CSS and JS release", () => {
  ...
  for (const system of Array.from(api.systems)) {
    ...
    assert.doesNotMatch(html, /shared-system-nav\.(?:css|js)\?v=(?!20260908-refine7)/, `${system.href} must not retain a stale shared navigation release`);
  }
