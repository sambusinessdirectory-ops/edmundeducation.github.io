...
assert.equal(HOMEWORK_RESOURCE_CATALOG.length, 4997, "the Homework/Schedule catalogue should include every current learning resource, downloadable file, Common Expression lesson, IELTS Listening part and learning portal");
...
assert.equal((byType["reading-analysis"] || []).length, 296, "all unique available IELTS Reading analyses should be indexed once");
...
assert.equal(mappedReadingCatalogueIds.size, 300, "296 unique analyses should cover all 300 currently available Passage 1 and Passage 2 catalogue positions");
