"""Generate every SHOW example with the established four voices, in manual order.
Credentials stay in memory. Cache immutable source hashes; pad starts for mobile playback.
"""
import argparse,json,hashlib,subprocess,tempfile,urllib.request
from pathlib import Path
p=argparse.ArgumentParser();p.add_argument('--model',required=True);p.add_argument('--voices',required=True);p.add_argument('--sentences',required=True);p.add_argument('--kind',choices=['local','cloud'],required=True);a=p.parse_args()
...
for index,row in enumerate(rows):
 voice=cycle[index%4];recipe=recipes[voice]
 ...
 if not dest.exists():
  with tempfile.TemporaryDirectory(prefix='polysemy-voice-') as temp:
   ...
   if a.kind=='local':
    ...
    if len(words)<2 or 'hello' not in words[0]['word'].lower():raise RuntimeError('Cannot safely remove generation prefix '+row['id'])
 ...
 manifest[row['id']]={'path':'audio/'+dest.name,'voice':voice,'index':index,'text':row['en'],'duration':round(duration,3),'sourceSha256':hashlib.sha256(row['en'].encode()).hexdigest()};print(row['id'],voice,round(duration,2),flush=True)
(root/f'polysemy-lab/audio-{a.kind}.json').write_text(json.dumps(manifest,indent=2)+'\n')
