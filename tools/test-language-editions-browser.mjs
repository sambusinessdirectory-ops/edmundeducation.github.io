...
for(const type of ['flashcards','writing-practice']){
 await open(type,'fr');assert.equal(await page.evaluate(()=>languageStorage.getItem('qa-edition-marker')),null);
...
console.log('PASS: six categories, 89 cards, 45 translations, 16 worksheet modes, audio timing/playback, isolated progress, French empty scopes, homepage switch');
await browser.close();
