...
SOURCE=ROOT/'docs/golden-manual-modular-wardrobe-v2.md'
DEST=ROOT/'output/pdf/Golden-Manual-Modular-Character-Wardrobe-SOP-v2.pdf'
...
def decorate(c,doc):
 ...
 if doc.page>1:
  ...
  c.drawRightString(w-52,h-28,'GOLDEN MANUAL  2.0')
 c.line(52,39,w-52,39);c.setFont('Helvetica',7);c.setFillColor(MUTED)
 c.drawString(52,27,'15 September 2026  |  Saved-avatar standard v2 / 20260915-avatar2')
...
story.append(Paragraph('Version 2.0 | 15 September 2026<br/>Eddy, Elsie and Phoebe<br/>Asset fitting, saved-avatar isolation and cross-system restoration',styles['body']))
...
lines=SOURCE.read_text().splitlines();start=next(i for i,l in enumerate(lines) if l.startswith('## 01.'));i=start
while i<len(lines):
 l=lines[i].strip()
