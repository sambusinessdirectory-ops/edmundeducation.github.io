...
assert(html.includes('<script src="flashcards-audio-manifest.js?v=edmund-neural-v1-20260920-civics-book3-1"></script>'), "Flashcard audio cache key is stale");
...
if (!dataOnly) {
  ...
  assert(
    ...
    `Expected at least ${expectedAudioCount.toLocaleString()} manifest entries, found ${audioMeta.count}`
  );
