...
Object.assign(w,{TextEncoder,Headers,AbortController,confirm:()=>false,...submit,previewEmail:async()=>({spellcheck:'passed'}),validateEmailDraft:()=>{},checkEmailSpelling:async()=>[]});
...
try {
 ...
 w.document.querySelector('[data-send]').click();await settle(()=>release);
 ...
 mode='lost';requests=[];w.document.querySelector('[data-send]').click();await settle(()=>requests.some(p=>p.includes('/requests/')));await settle(()=>!w.document.querySelector('[data-send]').disabled);
 ...
 mode='offline';w.document.querySelector('[data-send]').click();await settle(()=>!w.document.querySelector('[data-send]').disabled);
 assert.equal(w.document.querySelector('[data-submission-recovery]').hidden,false);
 const pendingKey=Object.keys(w.sessionStorage).find(k=>k.startsWith('edmund-email-pending-v3:'));
