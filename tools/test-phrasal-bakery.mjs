...
test('full Saturn rotation stays inside the sky and cup steam keeps its rim contact',()=>{
 const planet=BAKERY_OBJECTS.saturn,r=planet.size/Math.sqrt(2);assert.ok(planet.x-r>0&&planet.x+r<1600&&planet.y-r>=0);
 for(let i=0;i<3;i++){const paths=new Set();for(let t=0;t<20;t+=.5){const p=chocolateSteam(t,i);assert.ok(p.d.startsWith(`M${(i-1)*15} 0 `));assert.ok(!/NaN|Infinity/.test(p.d));assert.ok(p.opacity>=.32&&p.opacity<=.6);paths.add(p.d);}assert.ok(paths.size>30);}
});
