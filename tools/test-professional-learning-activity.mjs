...
// Upgrade existing aggregate completions, preserving dates, then test per-question scoring.
await db.exec(fs.readFileSync(new URL('../supabase/migrations/20260916124646_professional_team_momentum_history.sql',import.meta.url),'utf8'));
const upgrade=fs.readFileSync(new URL('../supabase/migrations/20260917072552_professional_polysemy_question_credit.sql',import.meta.url),'utf8');
const oldTotal=await scalar("select sum(points)::integer value from special_learning_events where kind='polysemy'");
const expectedTotal=await scalar("select sum((select count(*) from jsonb_object_keys(c.answers)))::integer value from special_learning_events e join special_learning_catalog c on c.kind=e.kind and c.exercise=(regexp_match(e.event_key,'^polysemy:(lesson-[0-9]+:[^:]+):(.+):word$'))[1] where e.kind='polysemy' and e.points>0");
await db.exec(upgrade);assert.ok(expectedTotal>oldTotal);assert.equal(await scalar("select sum(points)::integer value from special_learning_events where kind='polysemy'"),expectedTotal);
assert.equal(await scalar("select count(*)::integer value from special_learning_events e join special_learning_events old on old.account_id=e.account_id and old.event_key=regexp_replace(e.event_key,':[^:]+$',':word') where e.kind='polysemy' and e.points>0 and e.occurred_at<>old.occurred_at"),0,'historical dates preserved');
await db.exec(upgrade);assert.equal(await scalar("select sum(points)::integer value from special_learning_events where kind='polysemy'"),expectedTotal,'migration is idempotent');
const qaAnswers=Object.fromEntries(Array.from({length:26},(_,i)=>['q'+i,'sense'+i]));
await db.query("insert into special_learning_catalog(course_id,kind,exercise,answers) values($1,'polysemy','lesson-1:qa26',$2)",[course,JSON.stringify(qaAnswers)]);
const qs=Object.entries(qaAnswers).map(([item,answer])=>({kind:'polysemy',exercise:'lesson-1:qa26',attempt:'qa-run',item,answer}));
const before26=await summary(people[6]),teamBefore26=(await team(people[6])).courses[0].total_cards;
await activity(people[6],qs.slice(0,1));assert.equal((await summary(people[6])).questions-before26.questions,1,'immediate partial credit');
await assert.rejects(activity(people[6],[{...qs[1],answer:'wrong'}]),/not correct/);
await assert.rejects(activity(people[6],[{...qs[1],item:'invented'}]),/not correct/);
await activity(people[6],qs);await activity(people[6],qs);
assert.equal((await summary(people[6])).questions-before26.questions,26,'26 questions = 26 personal completions');
assert.equal((await team(people[6])).courses[0].total_cards-teamBefore26,26,'26 questions = 26 team completions');
const legacy26={kind:'polysemy',exercise:'lesson-1:qa26',attempt:'qa-run',item:'word',answers:qaAnswers};
assert.equal((await activity(people[6],[legacy26])).accepted,0,'cached whole-word submission cannot double count new question events');
await activity(people[6],[{...legacy26,attempt:'cached-client-run'}]);assert.equal((await summary(people[6])).questions-before26.questions,52,'cached old client receives 26 points for a new full run');
await activity(people[6],qs.map(e=>({...e,attempt:'cached-client-run'})));assert.equal((await summary(people[6])).questions-before26.questions,52,'new client cannot recount cached-client run');
...
const detail=await scalar('select special_flash_learning_details($1,$2,$3,$3,1) value',[people[6].token,course,hkDate]);
assert.equal(detail.rows.filter(r=>r.exercise==='lesson-1:qa26').length,52);assert.ok(detail.rows.filter(r=>r.exercise==='lesson-1:qa26').every(r=>r.item.startsWith('q')&&r.points===1&&!r.legacy));
await assert.rejects(activity(people[10],qs),/not available/);
console.log('Passed: per-question partial credit, 26-question personal/team totals, wrong answer rejection, retries, mixed old/new clients, historical conversion/date preservation, detail records and account isolation.');
