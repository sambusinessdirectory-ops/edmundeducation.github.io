...
const data=JSON.parse(read('professional-english/dialogues.json'));assert.equal(data.dialogues.length,23);assert.equal(data.dialogues.reduce((n,d)=>n+d.lines.length,0),242);
...
page.querySelector('[data-choose-mode]').click();await tick();assert.equal(page.querySelectorAll('[data-mode]').length,16);assert.equal(page.querySelectorAll('[data-learning-font]').length,1);page.querySelector('[data-mode="medium"][data-hints="first"]').click();await tick();assert.match(location.search,/view=practice/);const inputs=[...page.querySelectorAll('[data-blank]')];assert.ok(inputs.length>0);inputs.forEach((input,i)=>input.value=i?'wrong':input.dataset.answer);page.querySelector('[data-check-answers]').click();assert.match(page.querySelector('[data-result-status]').textContent,/1 \/ /);assert.equal(page.querySelectorAll('.is-correct').length,1);page.querySelector('[data-retry-mistakes]').click();assert.ok(inputs[0].value);assert.equal(inputs[1].value,'');app.stop();
...
console.log('Validated 800 listening questions, 3,102 speaking questions, four-corner geometry, finder filters/deep links, 242 translated lines, 16 modes, answer feedback and synchronized audio controls.');w.close();
