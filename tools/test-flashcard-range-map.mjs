...
assert.equal(layout.width,1280);assert.equal(layout.height,3744);
...
for(const node of layout.nodes) {
 ...
 for(const destination of layout.nodes) {
  ...
  assert.equal(route.length,1,'Every open-map journey takes the direct route');assert.deepEqual(route.at(-1),{x:destination.x,y:destination.y});
...
console.log('Flashcard map: forty compact destinations, 5/4 columns, all 1,600 direct routes, free walking and short decks passed.');
