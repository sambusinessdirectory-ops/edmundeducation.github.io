#!/usr/bin/env python3
"""Validate and upload the immutable Passage 2 flashcard audio release to R2."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "After every pack is uploaded and the index is marked complete, "
            "remove only the individual v1 MP3s represented by this release."
        ),
    )
...
def validate_packs(index_path: Path, pack_dir: Path) -> list[tuple[str, Path, str]]:
    ...
    if not isinstance(index, dict) or index.get("schemaVersion") != 1:
        raise SystemExit("Passage 2 flashcard pack index has an invalid schema")
    ...
    if not isinstance(packs, dict) or not isinstance(entries, dict) or not isinstance(
        ...
        raise SystemExit("Passage 2 flashcard pack index has an invalid shape")
    ...
    if (
        ...
        or not isinstance(index.get("cloudBaseUrl"), str)
        or not str(index["cloudBaseUrl"]).startswith("https://")
        ...
        raise SystemExit("Passage 2 flashcard pack index metadata is incomplete")
    for digest_name in (
        "corpusSha256",
        "sourceCorpusSha256",
        "existingIndexCorpusSha256",
    ):
        if not isinstance(meta.get(digest_name), str) or not re.fullmatch(
            r"[0-9a-f]{64}", meta[digest_name]
        ):
            raise SystemExit(f"Passage 2 pack metadata has invalid {digest_name}")
    ...
    for prefix, item in sorted(packs.items()):
        ...
        if (
            ...
            raise SystemExit(
                f"Unsafe or unexpected Passage 2 pack key for prefix {prefix!r}"
            )
    ...
    if indexed_recordings != entry_count or indexed_bytes != total_bytes:
        raise SystemExit(
            "Passage 2 flashcard pack index counts disagree with its validated packs"
        )
    return validated
...
def main() -> int:
    ...
    print(
        f"Passage 2 R2 source valid: {len(validated)} packs, "
    ...
    print(f"Passage 2 flashcard R2 release {RELEASE} upload complete.")
    if args.prune_source_audio:
        removed = prune_uploaded_source_audio(args.source_root.resolve(), index_path)
        print(
            f"Removed {removed:,} packed source MP3(s). Rebuild "
            "flashcards-audio-manifest.js before deployment."
        )
    return 0
