...
test("bookmarks remain above the first-150 map and completed 50-question lessons turn gold", () => {
  ...
  assert.equal(occurrences(cards, 'data-open-lesson='), 150);
  const remaining = selectorMap.get('[data-remaining-lesson-grid]').innerHTML;
  assert.equal(occurrences(remaining, 'data-open-lesson='), 195);
  assert.match(remaining, /data-open-lesson="ss151"/);
