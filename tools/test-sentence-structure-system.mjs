...
test("bookmarks remain above the first-180 map and completed 50-question lessons turn gold", () => {
  ...
  assert.equal(occurrences(cards, 'data-open-lesson='), 180);
  const remaining = selectorMap.get('[data-remaining-lesson-grid]').innerHTML;
  assert.equal(occurrences(remaining, 'data-open-lesson='), 165);
  assert.match(remaining, /data-open-lesson="ss181"/);
