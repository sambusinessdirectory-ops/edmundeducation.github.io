...
bindEvents();window.trophyTest={
 ...
 const shelf=page.locator('[data-sentence-trophy-shelf]'),map=page.locator('[data-sentence-map]');
 ...
 await shelf.locator('summary').click();assert.equal(await shelf.locator('[data-trophy-earned="true"]').count(),4);
 ...
 await page.locator('[data-view-sentence-trophies]').click();await page.waitForFunction(()=>document.querySelector('[data-sentence-trophy-shelf]').open&&!document.querySelector('[data-view=dashboard]').hidden);
 ...
 await page.setViewportSize({width:390,height:900});await shelf.locator('summary').click();await shelf.screenshot({path:path.join(out,'trophy-collection-phone.png'),animations:'disabled',style:'.site-header,[data-toast]{visibility:hidden!important}'});
