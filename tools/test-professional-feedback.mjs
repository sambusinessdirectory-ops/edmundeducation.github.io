...
vm.runInContext(enhancements.slice(enhancements.indexOf('  function smoothPath('),enhancements.indexOf('  function smoothDashboardCharts(')),context);
// Check every cubic control, and sampled curve values, stay within the adjacent data points.
for(const values of [[0,0,0,0,0,0,25],[25,0,0,0,0,0,25],[0,10,5,20,0,0],[0,0],[25,25]]){
 ...
 const path=context.smoothPath(points),numbers=path.match(/-?\d+(?:\.\d+)?(?:e[+-]?\d+)?/gi).map(Number);
 for(let i=1;i<points.length;i++){
  ...
  for(let step=0;step<=100;step++){
   ...
   assert.ok(y>=lo-1e-7&&y<=hi+1e-7,'curve must not invent values between samples');
  }
 }
}
const jsx=(type,props)=>typeof type==='function'?type(props):({type,props});
context.f={jsx,jsxs:jsx,Fragment:'fragment'};
context.EdmundTeamColors=['#c9f47d','#73c9ff'];
