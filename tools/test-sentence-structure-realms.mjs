...
test('180 real levels preserve coastal coordinates, labels and single-pin storage',()=>{
 assert.equal(lessons.length,180);assert.equal(catalogue.slice(180).length,165);assert.equal(lessons.at(-1).id,'ss180');
 ...
 assert.equal(restoreMapPreferences({pinned:'ss181'},null,lessons.map(l=>l.id)).pinned,null);
});
test('all 32400 ordered routes stay on dry land or a real bridge',()=>{
...
test('all six realms retain full painted overview coverage',()=>{
 assert.equal(SENTENCE_REALMS.width,1600);assert.equal(SENTENCE_REALMS.height,HOTEL_OFFSET+HOTEL_HEIGHT);assert.equal(SENTENCE_REALMS.minimumZoom,.5);assert.deepEqual(SENTENCE_REALMS.cameraPadding,{left:800,right:800});
