...
test('sixty real levels preserve coastal coordinates, labels and single-pin storage',()=>{
 assert.equal(lessons.length,60);assert.equal(catalogue.slice(60).length,285);assert.equal(lessons.at(-1).id,'ss60');
 ...
 assert.equal(restoreMapPreferences({pinned:'ss61'},null,lessons.map(l=>l.id)).pinned,null);
});
test('all 3600 ordered routes stay on dry land or a real bridge',()=>{
...
test('both realms retain full painted overview coverage',()=>{
 assert.equal(SENTENCE_REALMS.width,1600);assert.equal(SENTENCE_REALMS.height,3900);assert.equal(SENTENCE_REALMS.minimumZoom,.5);assert.deepEqual(SENTENCE_REALMS.cameraPadding,{left:800,right:800});
