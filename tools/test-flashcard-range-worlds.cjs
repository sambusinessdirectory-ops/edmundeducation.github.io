...
(async()=>{
 ...
 for(const world of ['standard','30','10'])await page.locator(`[data-range-world="${world}"]`).screenshot({path:path.join(out,`${world}-desktop.png`)});
 ...
 for(const world of ['standard','30','10'])await page.locator(`[data-range-world="${world}"]`).screenshot({path:path.join(out,`${world}-completed.png`)});
 ...
 await page.locator('[data-range-world="10"]').screenshot({path:path.join(out,'french-gems.png')});
 for(const width of [768,390]){
  await page.setViewportSize({width,height:1000});
  for(const world of ['standard','30','10'])await page.locator(`[data-range-world="${world}"]`).screenshot({path:path.join(out,`${world}-${width}.png`)});
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'No horizontal page overflow');
 }
 await page.setViewportSize({width:720,height:900});await visit('?language=ja&embedded=1&source=reading');
 await page.locator('.range-worlds').screenshot({path:path.join(out,'embedded-720.png')});
 await page.setViewportSize({width:390,height:844});
 await page.locator('.range-worlds').screenshot({path:path.join(out,'embedded-390.png')});
