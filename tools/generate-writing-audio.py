...
def main() -> int:
    ...
    for exercise_id, essay in essays.items():
        ...
        if args.force or not valid_existing_audio(output_path):
    ...
    if pending:
        ...
        for index, (exercise_id, essay, output_path) in enumerate(pending, start=1):
            ...
            sample_rate = 0
            for paragraph_index, paragraph in enumerate(essay["paragraphs"]):
                audio, chunk_rate = kokoro.create(
                    spoken_text(str(paragraph)),
                    voice=args.voice,
                    speed=args.speed,
                    lang=args.lang,
                )
                ...
                if paragraph_index < len(essay["paragraphs"]) - 1:
                    chunks.append(np.zeros(round(sample_rate * PARAGRAPH_PAUSE_SECONDS), dtype=np.float32))
