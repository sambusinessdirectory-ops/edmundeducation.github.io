...
const manualWritingResourceMigration = await read("supabase/migrations/20260901090000_schedule_create_manual_writing_resource.sql");
...
assert.match(scheduleHtml, /data-copy-unfinished-selection/);
assert.match(scheduleHtml, /data-manual-writing-topic-creator/);
assert.match(scheduleJs, /HOMEWORK_ENTRY_TAGS/);
assert.match(scheduleJs, /unfinishedOnly: true/);
assert.match(scheduleJs, /isScheduleClipboardNextWeekCarry/);
assert.match(scheduleJs, /schedule_admin_create_manual_writing_resource/);
...
assert.match(manualWritingResourceMigration, /security definer\s+set search_path = ''/i);
assert.match(manualWritingResourceMigration, /public\._schedule_admin_id\(p_admin_token\)/);
assert.match(manualWritingResourceMigration, /writing_submission_manual_topics/);
assert.match(manualWritingResourceMigration, /writing_submission_admin_id uuid/);
assert.match(manualWritingResourceMigration, /writing_admin\.id = schedule_admin\.writing_submission_admin_id/);
assert.match(manualWritingResourceMigration, /grant execute on function public\.schedule_admin_create_manual_writing_resource[\s\S]*?to authenticated/i);
assert.doesNotMatch(manualWritingResourceMigration, /grant (?:select|insert|update|delete|all) on table/i);
...
assert.equal(homepageCards.length, 64, "homepage must contain all 64 linked category cards, including Eddie Farm, Membership and Excellent Learning");
...
assert.equal(homepageCardHrefs[63], "excellent-learning-system.html", "Excellent Learning must be numbered card 64");
...
assert.match(scheduleHtml, /schedule-system\.js\?v=20260908-sunny-s3-1/);
...
assert.match(scheduleHtml, /現在先做自己喜歡的事，慶祝一下! 休息夠才繼續~/);
assert.match(scheduleHtml, /知道了，現在放鬆一下再回來 ~/);
assert.match(scheduleHtml, /assets\/schedule\/celebration-message\.webp/);
assert.ok((await stat(new URL("../assets/schedule/celebration-message.webp", import.meta.url))).size < 100_000,
  "the celebration artwork must remain lightweight enough for an immediate completion popup");
assert.match(scheduleHtml, /dialog\[data-celebration-dialog\]\s*\{[^}]*background:\s*#fbecdc/s);
assert.match(scheduleJs, /function showCompletionCelebration\(\)[\s\S]*?state\.currentUser\?\.role !== "student"/);
assert.match(scheduleJs, /if \(completed\) showCompletionCelebration\(\)/);
assert.match(scheduleJs, /active && targetStatus === "completed"\) showCompletionCelebration\(\)/);
...
assert.match(scheduleJs, /HOMEWORK_CATALOG_URL = "\.\/homework-resource-catalog\.mjs\?v=20260908-sunny-s3-1"/);
...
assert.match(scheduleJs, /schedule-homework-links\.mjs\?v=20260901-homework-workflow1/);
...
assert.match(scheduleJs, /state\.massEditMode[\s\S]*?state\.selectionMode[\s\S]*?event\.pointerType !== "mouse"/, "mouse marquee selection must work in both Mass Edit and ordinary multi-select mode");
assert.match(scheduleJs, /marquee\.mode === "selection"[\s\S]*?state\.selectedEntryIds = nextSelection/, "ordinary multi-select drag must update the selected homework entries");
