...
TABLE_BATCH_R2_PREFIX = "IELTS Writing Task 1/IELTS Writing Task 1 - Tables Batch 3"
...
def validate_inventory(entries: list[dict[str, object]]) -> None:
    if len(entries) != 62:
        raise ValueError(f"Expected 62 IELTS Task 1 PDFs, found {len(entries)}")
...
def main() -> None:
    ...
    parser.add_argument("source", type=Path, help="Folder containing the 33 canonical first-batch IELTS Task 1 PDFs")
    parser.add_argument(
        "--second-source",
        type=Path,
        required=True,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Folder containing Table model essays 2–11",
    )
    parser.add_argument(
        "--third-r2-prefix",
        default=TABLE_BATCH_R2_PREFIX,
        help="R2 prefix containing the Table batch PDFs",
    )
    parser.add_argument(
        "--legacy-variant-source",
        type=Path,
        required=True,
        help="Folder containing the two retained legacy variant PDFs",
    )
    ...
    third_source = args.third_source.expanduser().resolve()
    legacy_variant_source = args.legacy_variant_source.expanduser().resolve()
    site_root = args.site_root.expanduser().resolve()
    source_groups = [
        (source, R2_PREFIX, 1),
        (second_source, str(args.second_r2_prefix).strip("/"), 2),
        (third_source, str(args.third_r2_prefix).strip("/"), 3),
    ]
    pdfs = [
        (pdf, r2_prefix, batch)
    ...
    for filename in LEGACY_VARIANT_FILENAMES:
        pdf = legacy_variant_source / filename
        if not pdf.is_file():
            raise ValueError(f"Missing retained legacy Task 1 variant: {pdf}")
        pdfs.append((pdf, R2_PREFIX, 1))
