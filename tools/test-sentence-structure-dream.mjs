...
import {DREAM_RAIL,DREAM_OFFSET,dreamPositions} from '../sentence-structure-dream-geometry.mjs';
...
test('all train cars follow one closed track continuously, including the loop boundary',()=>{
 for(let car=0;car<3;car++)for(let t=0;t<145;t+=.03){
  ...
  assert.ok(Math.hypot(q.x-p.x,q.y-p.y)<.3);assert.ok(Math.abs(q.angle-p.angle)<.01);
 }
