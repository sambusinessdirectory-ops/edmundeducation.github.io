...
const source=fs.readFileSync(root+'/writing-submission.js','utf8')+'\nwindow.adminQA={state,elements,showView,renderAdminFeedbackEditor,openAdminPendingSubmissions,openAdminSubmission,closeFeedbackFullscreen,clearFeedbackSelectionRanges,createFeedbackRichEditor,readFeedbackRichEditor,appendFeedbackRichText};';
...
else if(path.endsWith('/feedback'))body={feedback:saved?{...saved,id:id(999),submissionId:id(1),version:1,fragments:[],status:saved.status}:null};
...
const editor=page.locator('[data-feedback-rich-editor="overall"]');
// Chinese-only and mixed selections must stay bold through toolbar use, save and reload.
const chinese='有人會說音樂創作一直都有用科技';
await editor.fill(chinese+'，English feedback.');
await editor.evaluate((el,length)=>{el.focus();const range=document.createRange();range.setStart(el.firstChild,0);range.setEnd(el.firstChild,length);getSelection().removeAllRanges();getSelection().addRange(range);},chinese.length);
await page.waitForFunction(()=>window.adminQA.state.feedbackSelectionRanges.length===1);
await page.locator('[data-feedback-selection-toolbar] [data-feedback-format="bold"]').click();
let formatted=await editor.evaluate(el=>window.adminQA.readFeedbackRichEditor(el));
assert.equal(formatted.text,chinese+'，English feedback.');assert.equal(formatted.formatting[0].bold,true);assert.equal(formatted.formatting[0].end,chinese.length);
await page.locator('[data-feedback-save="draft"]').first().click();
await page.waitForFunction(text=>window.adminQA.state.selectedAdminFeedback?.overallComment===text,chinese+'，English feedback.');
assert.equal(saved.overallFormatting[0].bold,true);assert.equal(saved.overallComment,chinese+'，English feedback.');
assert.equal(await editor.locator('strong').innerText(),chinese);
await editor.screenshot({path:path.join(artifacts,'writing-chinese-bold.png')});
// Compare actual glyph pixels, not just CSS's requested font-weight.
const ink=await editor.evaluate(async el=>{
 const family=getComputedStyle(el).fontFamily;
 const text='有人會說音樂創作一直都有用科技';
 await document.fonts.load('400 40px '+family,text).catch(()=>{});
 await document.fonts.load('700 40px '+family,text).catch(()=>{});
 const images=[400,700].map(weight=>{const canvas=document.createElement('canvas');canvas.width=800;canvas.height=90;const ctx=canvas.getContext('2d');ctx.font=weight+' 40px '+family;ctx.fillText(text,5,60);return ctx.getImageData(0,0,800,90).data;});
 let regular=0,bold=0,different=0;for(let i=3;i<images[0].length;i+=4){regular+=images[0][i];bold+=images[1][i];if(images[0][i]!==images[1][i])different++;}return{regular,bold,different};
});
assert.ok(ink.different>100&&ink.bold>ink.regular*1.03,'Chinese bold must visibly differ from regular text');
await editor.evaluate(el=>{el.replaceChildren();window.adminQA.clearFeedbackSelectionRanges();});
await editor.fill('Delete this line. Keep this sentence.');
...
await page.waitForFunction(()=>window.adminQA.state.selectedAdminFeedback?.overallComment==='Unsaved feedback survives fullscreen.');
