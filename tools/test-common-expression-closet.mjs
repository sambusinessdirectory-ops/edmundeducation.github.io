...
test('closet UI is responsive and cache-busted on every shared-map consumer', () => {
  ...
  for (const file of consumers) assert.match(read(file), /common-expression-map\.(?:css\?v=20260915-noirceleste1|mjs\?v=20260916-girls-fleece1)/, file);
  assert.match(read('ielts-puzzle-map.mjs'), /common-expression-map\.mjs\?v=20260916-girls-fleece1/);
  for (const file of [
  ...
  ]) assert.match(read(file), /\.js\?v=20260916-girls-fleece1/, file);
