...
test('closet UI is responsive and cache-busted on every shared-map consumer', () => {
  ...
  for (const file of consumers) assert.match(read(file), /common-expression-map\.(?:css\?v=20260915-avatar2|mjs\?v=20260915-avatar2)/, file);
  assert.match(read('ielts-puzzle-map.mjs'), /common-expression-map\.mjs\?v=20260915-avatar2/);
  for (const file of [
  ...
  ]) assert.match(read(file), file === 'common-expression-business-speaking.html' ? /common-expression-system\.js\?v=20260915-lounge3/ : /\.js\?v=20260915-avatar2/, file);
});
