...
test('closet UI is responsive and cache-busted on every shared-map consumer', () => {
  ...
  for (const file of consumers) assert.match(read(file), /common-expression-map\.(?:css\?v=20260915-closet2|mjs\?v=20260915-closet2)/, file);
  assert.match(read('ielts-puzzle-map.mjs'), /common-expression-map\.mjs\?v=20260915-closet2/);
  for (const file of [
  ...
  ]) assert.match(read(file), file === 'common-expression-business-speaking.html' ? /common-expression-system\.js\?v=20260915-lounge2/ : /\.js\?v=20260915-closet2/, file);
});
