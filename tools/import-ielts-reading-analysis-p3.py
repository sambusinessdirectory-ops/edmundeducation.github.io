#!/usr/bin/env python3
"""Import Edmund's Passage 3 analysis PDFs into deterministic website JSON.
...
Usage:
  python tools/import-ielts-reading-analysis-p3.py --check
  python tools/import-ielts-reading-analysis-p3.py --write
  python tools/import-ielts-reading-analysis-p3.py --write --source-dir /path/to/pdfs
"""
...
MANIFEST_PATH = ROOT / "tools/ielts-reading-analysis-p3-import-manifest.json"
REPORT_PATH = ROOT / "tools/ielts-reading-analysis-p3-import-report.json"
OUTPUT_DIR = ROOT / "ielts-reading-analysis-data"
AVAILABILITY_PATH = ROOT / "ielts-reading-analysis-availability.js"
IMPORT_VERSION = "2026-08-22.1"
AVAILABILITY_VERSION = "2026-08-22.1"
...
GROUP_LINE = re.compile(
    r"^(?:\d+\.\s*)?(?:(?:Part|Section)\s+\d+\s*(?:[—–-]|[:：])\s*)?Questions?\s+"
    r"(\d+)(?:\s*[–—-]\s*(\d+))?\s*[:：]?\s*(.*)$",
    re.IGNORECASE,
)
PARAGRAPH_LINE = re.compile(
    r"^(?:Paragraph|Section)\s+([A-Z]|\d+)\s*(?:Roadmap)?\s*$", re.IGNORECASE
)
PARAGRAPH_INLINE = re.compile(
    r"^(?:Paragraph|Section)\s+([A-Z]|\d+)\s*(?:Roadmap)?\s*[:：—–-]\s*(.+)$",
    re.IGNORECASE,
)
...
OVERVIEW_HEADING_LINE = re.compile(
    r"^\s*(?:(?:\d+\.)|(?:[一二三四五六七八九十]+[、．.]))?\s*"
    r"(?:Skim(?:\s+Roadmap|[:：]\s*(?:全文\s+)?(?:Section\s+)?Roadmap)|"
    r"Paragraph\s+Map|(?:全文|文章)?段落(?:地圖|對應表))",
    re.IGNORECASE,
)
...
def is_answer_heading(line: str) -> bool:
    ...
    return (
        compact.endswith(("答案總表", "答案表", "answerkey"))
        or bool(re.search(r"(?:答案總表|答案表|Answer\s*Key)", line, re.IGNORECASE))
    )
...
def answer_region(lines: list[str]) -> tuple[int, int] | None:
    ...
    for i in range(start + 1, len(lines)):
        ...
        if (
            ...
            or line in {"段落地圖", "全文段落地圖", "段落對應表"}
            or OVERVIEW_HEADING_LINE.search(line)
...
def derive_answer_key_from_questions(lines: list[str]) -> tuple[int, list[str]]:
    """Recover a missing summary table from explicit per-question answer lines.

    This path is enabled only by a deterministic manifest override. It never
    infers an answer from prose: every question must have a nearby authored
    ``答案：...`` line, and the resulting question numbers must be contiguous.
    """
    ...
    if not markers:
        raise ValueError("No individual question markers found for answer-key recovery")

    answers: dict[int, str] = {}
    for position, (line_index, number) in enumerate(markers):
        next_index = markers[position + 1][0] if position + 1 < len(markers) else len(lines)
        answer = next(
            (
                match.group(1).strip()
                for line in lines[line_index + 1 : next_index]
                if (match := re.match(r"^答案\s*[:：]\s*(.+)$", line))
            ),
            None,
        )
        if answer is None:
            raise ValueError(f"Question {number} has no explicit answer line")
        answers[number] = re.split(r"\s+[—–-]\s+", answer, maxsplit=1)[0].strip()

    minimum = min(answers)
    maximum = max(answers)
    expected = list(range(minimum, maximum + 1))
    if sorted(answers) != expected:
        raise ValueError("Recovered answer key is not contiguous")
    return minimum, [answers[number] for number in expected]


def parse_answer_key_with_prefix(
    lines: list[str],
    prefix_answers: list[str],
) -> tuple[int, list[str]]:
    """Combine verified heading answers with the table's numbered rows."""
    ...
    for line in lines[start + 1 : end]:
        ...
        if match and match.group(2) is None:
            rows.append((int(match.group(1)), [match.group(3).strip()]))
            active_row = len(rows) - 1
            continuation_count = 0
            continue
        if (
            ...
            and not re.search(r"[\u3400-\u9fff]", line)
            and not re.search(r"[。！？!?]", line)
            and not re.match(r"^(?:Paragraph|Questio|n|Question|題目)", line, re.I)
        ):
            rows[active_row][1].append(line)
            continuation_count += 1
        else:
            active_row = None

    if not rows:
        raise ValueError("Answer key contains no numbered rows after heading prefix")
    numbered = {number: smart_join(parts) for number, parts in rows}
    minimum = min(numbered)
    maximum = max(numbered)
    expected = list(range(minimum, maximum + 1))
    if sorted(numbered) != expected:
        raise ValueError("Numbered answer-key suffix is not contiguous")
    question_start = minimum - len(prefix_answers)
    if question_start <= 0:
        raise ValueError("Answer-key prefix produces an invalid starting question")
    return question_start, list(prefix_answers) + [numbered[number] for number in expected]
...
def clean_overview_heading(value: str) -> str:
    return re.sub(
        r"^\s*(?:(?:\d+\.)|(?:[一二三四五六七八九十]+[、．.]))\s*",
        "",
        value,
...
def parse_paragraph_table_overview(
    preamble: list[str],
    heading_index: int | None,
    analysis_start: int,
) -> tuple[dict[str, Any], list[str]] | None:
    """Parse the compact Chinese two-column paragraph-map template."""
    ...
    header_index = next(
        (
            i
            for i in range(heading_index + 1, min(heading_index + 5, len(preamble)))
            if (
                (
                    preamble[i].startswith(("Paragraph", "Paragrap"))
                    and "段落功能" in preamble[i]
                )
                or re.sub(r"\s+", "", preamble[i]) == "段落功能"
            )
        ),
        None,
    )
    ...
    table_end = next(
        (
            i
            for i in range(header_index + 1, min(analysis_start, len(preamble)))
            if is_answer_heading(preamble[i])
        ),
        min(analysis_start, len(preamble)),
    )

    row_starts: list[tuple[int, str, str, int | None]] = []
    for i in range(header_index + 1, table_end):
        row = re.match(r"^Paragraph\s+(.+)$", preamble[i], re.IGNORECASE)
        direct_letter = re.match(r"^([A-Z])\s+(.+)$", preamble[i])
        if not row and direct_letter:
            row_starts.append((i, direct_letter.group(1), direct_letter.group(2).strip(), None))
            continue
        if not row:
            continue
        first_text = row.group(1).strip()
        direct = re.match(r"^([A-Z]|\d+)\s+(.+)$", first_text, re.IGNORECASE)
        if direct:
            row_starts.append((i, direct.group(1).upper(), direct.group(2).strip(), None))
            continue
        number_line: int | None = None
        number = ""
        for candidate_index in range(i + 1, min(i + 4, table_end)):
            candidate = re.match(r"^([A-Z]|\d+)\s+(.+)$", preamble[candidate_index], re.IGNORECASE)
            if candidate:
                number_line = candidate_index
                number = candidate.group(1).upper()
                break
            if preamble[candidate_index].startswith("Paragraph "):
                break
        if number_line is not None:
            row_starts.append((i, number, first_text, number_line))

    if len(row_starts) < 2:
        return None

    paragraphs: list[dict[str, str]] = []
    for position, (line_index, number, first_text, number_line) in enumerate(row_starts):
        next_index = row_starts[position + 1][0] if position + 1 < len(row_starts) else table_end
        parts = [first_text]
        for i in range(line_index + 1, next_index):
            line = preamble[i]
            if i == number_line:
                line = re.sub(r"^(?:[A-Z]|\d+)\s+", "", line, count=1, flags=re.IGNORECASE)
            parts.append(line)
        summary = smart_join(parts)
        if summary:
            paragraphs.append({"number": number, "summary": summary})
    if len(paragraphs) < 2:
        return None

    source_notes: list[str] = []
    last_answer_row = answer_table_last_row(preamble, analysis_start)
    if last_answer_row is not None:
        source_notes = source_note_chunks(preamble[last_answer_row + 1 : analysis_start])
    source_notes = [note for note in source_notes if len(note.strip()) >= 10]
    return ({
        "title": clean_overview_heading(preamble[heading_index]),
...
def parse_overview(
    ...
    table_overview = parse_paragraph_table_overview(
        preamble,
        heading_index,
        analysis_start,
    )
    if table_overview is not None:
    ...
    if heading_index is not None:
        ...
        for line in preamble[heading_index + 1 : first_start]:
            ...
            if (
                ...
                and not re.search(r"[。！？.!?]$", line)
                and bool(re.search(r"(?:Roadmap|Map|Overview|GIVEN|地圖|段落|對應)", line, re.I))
            ):
                heading_lines.append(line)
            else:
                in_heading_extra = True
                heading_extra_lines.append(line)
        heading = clean_overview_heading(smart_join(heading_lines))
    else:
...
def parse_groups(lines: list[str]) -> tuple[list[Group], int]:
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r"^Question\s+\d+", line, re.I) and not re.match(r"^Questions\s+", line, re.I):
            ...
            if not next_marker or int(next_marker.group(1)) != int(match.group(1)):
                continue
        prefix = bool(re.match(r"^(?:\d+\.\s*|(?:Part|Section)\s+\d+)", line, re.I))
        has_colon = bool(
            re.match(
                r"^(?:\d+\.\s*)?(?:(?:Part|Section)\s+\d+\s*(?:[—–-]|[:：])\s*)?"
    ...
    # Some analyses introduce one broad range and immediately divide it into
    # titled subranges (for example Questions 29-34, then 29-31 and 32-34).
    # The broad line is an umbrella heading, not a second analysis unit. Drop
    # it only when later nested ranges cover every question exactly.
    umbrella_indexes: set[int] = set()
    for position, (_, match) in enumerate(boundaries):
        start = int(match.group(1))
        end = int(match.group(2) or start)
        if end <= start:
            continue
        covered: set[int] = set()
        for _, later in boundaries[position + 1 :]:
            later_start = int(later.group(1))
            later_end = int(later.group(2) or later_start)
            if later_start > end:
                break
            if start <= later_start <= later_end <= end:
                covered.update(range(later_start, later_end + 1))
        if covered == set(range(start, end + 1)):
            umbrella_indexes.add(position)
    if umbrella_indexes:
        boundaries = [
            boundary
            for position, boundary in enumerate(boundaries)
            if position not in umbrella_indexes
        ]
    ...
    analysis_start = first_boundary_index
    early_markers = [
        (i, int(match.group(1)))
...
def parse_article(
    ...
    if source.get("answerKeyPrefix"):
        question_number_start, answer_key = parse_answer_key_with_prefix(
            lines,
            source["answerKeyPrefix"],
        )
    elif source.get("deriveAnswerKey"):
        question_number_start, answer_key = derive_answer_key_from_questions(lines)
    else:
    ...
    if source.get("overviewUnavailable"):
        article["overviewUnavailable"] = True
...
def validate_article(article: dict[str, Any]) -> list[str]:
    ...
    if (
        not article.get("paragraphOverview", {}).get("paragraphs")
        and not article.get("overviewUnavailable")
    ):
        errors.append(f"{label}: paragraph overview is missing")
...
def write_availability(articles: list[dict[str, Any]]) -> None:
    ...
    # Preserve every Passage 1 and 2 (and future non-Passage-3) analysis while
    # replacing Passage 3 as one deterministic corpus.
    entries: dict[str, Any] = {
        article_id: entry
        for article_id, entry in previous.get("articles", {}).items()
        if entry.get("passage") != 3
    }
...
def main() -> int:
    ...
    if args.write:
        ...
        # hand-authored JSON that does not begin with a Passage 3 catalogue id.
        for path in OUTPUT_DIR.glob("p3-*.json"):
