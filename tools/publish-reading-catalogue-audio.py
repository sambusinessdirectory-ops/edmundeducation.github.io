...
def publish(root: Path, output: Path, articles: list[dict], records: dict, verified: dict, recipe: dict) -> dict:
    ...
    missing = [payload["id"] for payload in articles if payload["id"] not in existing and payload["id"] not in records]
    if missing:
    ...
    for payload in articles:
        ...
        if article_id in existing:
            batch.validate_entry(payload, batch.expanded_entry(root, article_id, existing[article_id]))
            continue
    ...
    meta = {**old_meta, **{key: recipe[key] for key in ("buildVersion", "voice", "language", "speed", "sampleRate", "format", "wordTiming", "recipeSha256")},
