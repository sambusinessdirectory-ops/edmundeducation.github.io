...
def publish(root: Path, output: Path, articles: list[dict], records: dict, verified: dict, recipe: dict) -> dict:
    ...
    for payload in articles:
        ...
        timing_path = f"{batch.TIMING_DIRECTORY}/{article_id}-{record['audioSha256'][:24]}.json"
    ...
    content += f"window.{batch.MANIFEST_NAMES[0]} = Object.freeze({json.dumps(entries, ensure_ascii=False, separators=(',', ':'))});\n"
    content += f"window.{batch.MANIFEST_NAMES[1]} = Object.freeze({json.dumps(meta, separators=(',', ':'))});\n"
    ...
    timing_directory = (root / batch.TIMING_DIRECTORY).resolve()
    removed_timings = 0
    for timing_path in old_timing_paths - active_timing_paths:
        ...
        if candidate.is_file():
            candidate.unlink()
            removed_timings += 1
    catalogue_path = root / ("dse-reading-catalogue.json" if batch.SYSTEM == "dse" else "reading-comprehension-catalogue.json")
    catalogue = json.loads(catalogue_path.read_text())
    rows = ([entry for year in catalogue["years"] for entry in year["sections"].values() if entry]
            if batch.SYSTEM == "dse" else catalogue["articles"])
    for row in rows:
...
def main() -> int:
    ...
    batch.configure_system(args.system)
    ...
    if failures:
        ...
        return 1
    (output / "r2-failures.json").unlink(missing_ok=True)
    if args.publish:
