...
export function createListeningTrophyProgress({rpc, getOwner, getToken, onChange}) {
  ...
  function reset() { records = {}; dates = {}; owner = ''; }
  function save() { try { if(owner) localStorage.setItem(key(owner),JSON.stringify({records,dates})); } catch {} }
  function answersFor(id) { return Object.fromEntries(clean(records[id]).map(n=>[String(n),{correct:true,updatedAt:dates[id]?.[n] || ""}])); }
  async function sync(practice, ids = []) {
    const requestOwner = getOwner(), token = getToken();
    if (!requestOwner || !token) return;
    const rows = await rpc('ielts_trophy_progress_v2', {p_token:token,p_practice:practice || null,p_correct_ids:clean(ids)});
    ...
    for (const row of rows || []) {
      ...
      dates[id] ||= {};
      for (const [n,date] of Object.entries(row.correct_dates || {})) if(Number.isFinite(Date.parse(date))) dates[id][n] = dates[id][n] && Date.parse(dates[id][n])<Date.parse(date) ? dates[id][n] : date;
    }
    save(); onChange();
  }
  async function restore() {
    reset(); owner = getOwner(); if(!owner) return;
    try { const saved=JSON.parse(localStorage.getItem(key(owner)) || '{}'); dates=saved.dates || {}; for(const [id,ids] of Object.entries(saved.records || saved)) if(/^ielts-listening-practice-\d+$/.test(id)) records[id]=clean(ids); } catch {}
    onChange();
  ...
  async function record(practice, ids) {
    ...
    dates[id] ||= {}; const now=new Date().toISOString(); for(const n of clean(ids)) if(!(records[id] || []).includes(n)) dates[id][n]=now;
