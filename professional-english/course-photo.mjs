...
export function mountCoursePhoto(host,owner,preferredCourse=''){
 ...
 host.className='course-photo-slot';host.hidden=true;
 ...
 async function publish(image){if(!owns()||uploading)return;uploading=true;const id=selected;host.querySelectorAll('button,select').forEach(b=>b.disabled=true);notice('正在更新課程照片…');try{const row=await rpc('course_photo',{p_course:id,p_action:image?'save':'remove',p_image:image,p_revision:revision},owner);if(!owns()||selected!==id)return;currentImage=row.image||null;revision=row.revision;render(row.can_edit);notice('已更新，同課程的學生可看到。');}catch(error){if(owns()){notice(error.message+' 照片未更新，請重新整理後再試。');}}finally{uploading=false;if(owns())host.querySelectorAll('button,select').forEach(b=>b.disabled=false);}}
