...
const HAS_LESSON_MAP = ["speaking", "written", "rhetorical-speaking", "rhetorical-writing", "business-speaking"].includes(SYSTEM_KEY);
...
function syncExpressionMap() {
  ...
  expressionMapModule ||= Promise.all([
    ...
    SYSTEM_KEY === "business-speaking" ? import("./common-expression-airport.mjs?v=20260914-airport1").then(async module => { await module.prepareAirport(); return module; }) :
  ...
  expressionMapModule.then(([{ createExpressionMap }, scene]) => {
    ...
    expressionMap ||= createExpressionMap({
      ...
      lessons: scene?.blueprintLessons ? scene.blueprintLessons(SYSTEM.lessons) : SYSTEM.lessons,
      systemKey: SYSTEM_KEY,
      theme: scene?.BUSINESS_AIRPORT || scene?.WRITTEN_GARDEN || scene?.RHETORICAL_COAST || scene?.RHETORICAL_BLUEPRINT,
