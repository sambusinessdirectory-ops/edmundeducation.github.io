...
export async function storeRecording(item){await stored(item.owner+':record:'+item.id,item);}
export async function listRecordings(owner){const d=await db();return new Promise((resolve,reject)=>{const tx=d.transaction('recordings','readonly'),rows=[],r=tx.objectStore('recordings').openCursor();r.onsuccess=()=>{const c=r.result;if(!c)return;const k=String(c.key);if(k.startsWith(owner+':')){if(c.value?.id)rows.push(c.value);else if(c.value instanceof Blob){const parts=k.split(':');rows.push({id:null,owner,run:parts[1],slot:parts[2],blob:c.value,legacy:true,at:null});}}c.continue();};tx.oncomplete=()=>resolve(rows);tx.onerror=()=>reject(tx.error);});}
