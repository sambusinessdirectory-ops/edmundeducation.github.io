...
export function closeTurn(s,now=Date.now()) {
  ...
  if(turn){closeSegment(s,turn,now);turn.endedAt=Math.max(turn.startedAt,Math.min(now,s.pausedAt??s.deadline??now));turn.durationMs=Math.max(0,turn.endedAt-turn.startedAt+(turn.adjustmentMs||0));}
  s.activeTurn=null;
}
...
export function turnDuration(s,turn,now=Date.now()){return turn.endedAt==null?Math.max(0,Math.min(now,s.pausedAt??s.deadline??now)-turn.startedAt+(turn.adjustmentMs||0)):turn.durationMs;}
...
function closeSegment(s,turn,now){const part=turn.segments?.find(p=>p.id===turn.activeSegment);if(part){part.endedAt=Math.max(part.startedAt,Math.min(now,s.pausedAt??s.deadline??now));part.durationMs=part.endedAt-part.startedAt;}turn.activeSegment=null;}
...
export function turnPartMs(s,turn,kind,now=Date.now()){return (turn.segments||[]).filter(p=>p.kind===kind).reduce((n,p)=>n+(p.endedAt==null?Math.max(0,Math.min(now,turn.endedAt??s.pausedAt??s.deadline??now)-p.startedAt):p.durationMs),0);}
...
export function overlapMs(s,turn=null,now=Date.now()){return (s.overlaps||[]).reduce((n,o)=>n+Math.max(0,Math.min(o.endedAt??s.pausedAt??s.deadline??now,turn?.endedAt??s.pausedAt??s.deadline??now,now)-Math.max(o.startedAt,turn?.startedAt??0)),0);}
...
export function toggleTimerPause(s,now=Date.now()){if(!['group','individual'].includes(s.phase))return false;if(s.pausedAt==null){s.pausedAt=now;return true;}const shift=Math.max(0,now-s.pausedAt);if(s.deadline!=null)s.deadline+=shift;if(s.groupStartedAt!=null)s.groupStartedAt+=shift;s.pausedAt=null;return true;}
