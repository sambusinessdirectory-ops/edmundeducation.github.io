...
import {mapperElapsed,mapperStop,mapperSections} from './execution-time-mapper-core.mjs?v=20260914-3';
...
export function initTimeMapper({key,canOpen,rpc,onSaved,message}) {
  ...
  dialog.innerHTML=`<header><div><span class="eyebrow">TIME MAPPER</span><h2 id="mapper-heading">邊做邊建立計時器</h2></div><button type="button" data-map-close aria-label="暫存並關閉">暫存並關閉</button></header>
    ...
    <footer><button type="button" class="primary" data-map-finish hidden>完成並儲存計時器與紀錄 ✓</button><p class="muted">實測紀錄保留精確時間；未來挑戰的預計時間向上取整至秒（最少 1 秒）。完成前的草稿會暫存在此裝置。</p></footer>`;
  ...
  function render() {
    ...
    $('[data-map-parts]').innerHTML=mapperSections(draft.parts).map(s=>`<div class="mapper-section"><strong>${escape(s.title)}</strong><b>${formatTime(draft.parts.filter(p=>p.section_id===s.id).reduce((n,p)=>n+p.elapsed_ms,0),true)}</b>${s.items.length ? s.items.map(i=>`<div><span>${escape(i.title)}</span><b>${formatTime(draft.parts.find(p=>p.id===i.id).elapsed_ms,true)}</b></div>`).join('') : ''}</div>`).join('');
  ...
  $('[data-map-form]').addEventListener('submit',e=>{
    ...
    const same=draft.parts.length>0 && $('[data-map-choice]').value==='same', previous=draft.parts.at(-1);
    ...
    if(draft.parts.length>=500 || !same && sections.length>=50 || same && sections.at(-1).items.length>=100)return error('最多 50 個主項目、每個主項目 100 個子項目，合共 500 個計時部分。請完成並儲存。');
    const now=Date.now();
  ...
  $('[data-map-finish]').addEventListener('click',async()=>{
    if(saving)return;
    draft=mapperStop(draft);draft.finished=true;persist();saving=true;error();render();
    try {
      const saved=await rpc('execution_speedrun_mapper_save',{p_id:draft.id,p_run_id:draft.run_id,p_title:draft.title,p_sections:mapperSections(draft.parts),p_splits:draft.parts.map(p=>({elapsed_ms:p.elapsed_ms,started_at:p.started_at,ended_at:p.ended_at}))});
