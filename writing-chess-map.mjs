...
export function mountWritingChessMap(host,{storage,owner,exerciseId,onStart}={}) {
  ...
  const drawSprite=(target,id,facing,time,walking)=>{
    ...
    if(!img){img=new Image();images.set(id,img);img.onload=()=>{if(!dead){drawCast();draw(performance.now());}};img.onerror=()=>{if(!dead)status.textContent='角色圖片未能載入；仍可選擇並開始練習。';};img.src=new URL(`./assets/speaking-system/mascots/v2/${MASCOT_VIEWS[id].standing.image}`,import.meta.url).href;}
