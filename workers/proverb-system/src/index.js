...
export default {
  async fetch(request, env) {
    try {
      ...
      console.error("Proverb System Worker request failed", safeErrorMessage(error));
      return json(
        { error: "Proverb System service error", code: "SERVICE_ERROR" },
        500,
        request,
        env
      );
...
function assertConfigured(env) {
  if (!isConfigured(env)) {
    throw new HttpError(503, "NOT_CONFIGURED", "Proverb System service is not configured");
  }
}
...
async function rpc(env, functionName, payload) {
  ...
  try {
    ...
    throw new HttpError(
      ...
      "Proverb System data service returned an invalid response"
    );
  }
}
...
async function authenticateStudent(request, env) {
  ...
  const row = singleRow(await rpc(env, "proverb_system_student_profile", { p_token: token }));
...
async function authenticateAdmin(request, env) {
  const token = bearerToken(request);
  if (!token) return null;
  const row = singleRow(await rpc(env, "proverb_system_admin_me", { p_admin_token: token }));
  if (!row || !UUID_RE.test(String(row.id || ""))) return null;
  return {
    kind: "admin",
    id: String(row.id).toLowerCase(),
...
async function adminLogin(request, env) {
  ...
  try {
    rateLimit = await env.ADMIN_LOGIN_RATE_LIMITER.limit({
      key: `proverb-system-admin:${clientIp}`
    });
  } catch {
  ...
  const row = singleRow(await rpc(env, "proverb_system_admin_login", {
    p_name: username,
    p_password: password
  }));
...
async function adminLogout(request, env) {
  const admin = await authenticateAdmin(request, env);
  if (!admin) throw new HttpError(401, "ADMIN_AUTH_REQUIRED", "Administrator authentication required");
  await rpc(env, "proverb_system_admin_logout", { p_admin_token: admin.token });
...
function normalizeAttemptResult(value, context) {
  ...
  if (correctIds.length && !answerCatalogReadiness().ready) {
    throw new HttpError(
      ...
      "Proverb System answer validation is not configured"
    );
  }
...
async function listAttempts(request, env, url) {
  ...
  const rows = await rpc(env, "proverb_system_list_attempts", {
...
async function getAttempt(request, env, attemptId) {
  ...
  const student = await authenticateStudent(request, env);
  if (!student) throw new HttpError(401, "STUDENT_AUTH_REQUIRED", "Student authentication required");
  const row = singleRow(await rpc(env, "proverb_system_get_attempt", {
    p_student_id: student.id,
...
async function putAttempt(request, env, attemptId) {
  ...
  const row = singleRow(await rpc(env, "proverb_system_upsert_attempt", {
    p_id: attemptId.toLowerCase(),
    p_student_id: student.id,
...
async function enforceAttemptWriteRateLimit(studentId, env) {
  ...
  try {
    result = await env.ATTEMPT_WRITE_RATE_LIMITER.limit({
      key: `proverb-system-attempt:${studentId}`
    });
  } catch {
...
async function getBookmarks(request, env) {
  const student = await authenticateStudent(request, env);
  if (!student) throw new HttpError(401, "STUDENT_AUTH_REQUIRED", "Student authentication required");
  const rows = await listAllBookmarkRows(env, "proverb_system_list_bookmarks_page", {
    p_student_id: student.id
  });
...
async function putBookmarks(request, env) {
  ...
  const replacedRows = await rpc(env, "proverb_system_replace_bookmarks", {
    p_student_id: student.id,
...
async function listAdminStudents(request, env) {
  const admin = await authenticateAdmin(request, env);
  if (!admin) throw new HttpError(401, "ADMIN_AUTH_REQUIRED", "Administrator authentication required");
  const rows = await rpc(env, "proverb_system_admin_list_students", {
...
async function getAdminStudent(request, env, studentId) {
  ...
  const [profileRows, attemptRows, bookmarkRows] = await Promise.all([
    rpc(env, "proverb_system_admin_student_profile", {
      p_admin_token: admin.token,
      p_student_id: normalizedStudentId
    }),
    rpc(env, "proverb_system_admin_list_attempts", {
      p_admin_token: admin.token,
      p_student_id: normalizedStudentId,
      p_limit: MAX_ADMIN_ATTEMPTS
    }),
    listAllBookmarkRows(env, "proverb_system_admin_list_bookmarks_page", {
      p_admin_token: admin.token,
      p_student_id: normalizedStudentId
    })
  ]);
