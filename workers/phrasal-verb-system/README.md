...
second student account or password table. Phrasal Verb attempts, bookmarks, admin
accounts, and admin sessions remain separately namespaced as
`phrasal_verb_system_*`.

All Phrasal Verb System tables have RLS enabled with no permissive browser policies.
...
`../../supabase-phrasal-verb-system.sql` in a private Supabase SQL session.
...
first = getpass.getpass("Phrasal Verb System admin password: ").encode()
...
```sql
select *
from public.phrasal_verb_system_provision_admin(
  'Sam Phrasal Verb Admin',
  '<PASTE_COST_12_BCRYPT_HASH_ONLY>'
);
```

The provisioning RPC is unavailable to `service_role`. Calling it again
rotates the hash and revokes every earlier Phrasal Verb System admin session.
...
Add any staging origin explicitly; never use `*`. The Phrasal Verb Worker owns rate
limit namespaces `914072036`, `914072037`, and `914072038`, distinct from every existing
...
writes and bookmark writes are each limited to 30 per student per 60 seconds.
...
accept only the separate token returned by Phrasal Verb System admin login.
...
attempts and all Phrasal Verb bookmarks. A Phrasal Verb System administrator never
