...
For an existing production installation that already supports `ss1`–`ss275`,
apply only the latest forward migration
`../../supabase-sentence-structure-lessons-276-345.sql`. It widens the permitted
...
validated bearer token. Only catalog version `1`, lessons `ss1`–`ss345`, their
...
20,000 items are rejected. Existing bookmark creation timestamps survive updates.
