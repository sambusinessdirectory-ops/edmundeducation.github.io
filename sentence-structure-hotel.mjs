import {HOTEL_OFFSET,HOTEL_HEIGHT,HOTEL_SCALE,HOTEL_ROOMS,hotelCompanionVisible} from './sentence-structure-hotel-geometry.mjs?v=20260914-hotel1';
import {makeHotelPlate,createHotelScenery,createHotelEffects} from './sentence-structure-hotel-scenery.mjs?v=20260914-hotel1';
const ART='./assets/sentence-structure/hotel/';
export function hotelTerrain(){return `<div class="sentence-hotel-realm" style="top:${HOTEL_OFFSET}px;height:${HOTEL_HEIGHT}px" aria-hidden="true"><img class="hotel-background" src="${ART}reference.png" width="1402" height="1122" alt=""><canvas class="hotel-scenery" width="1402" height="1122"></canvas><canvas class="hotel-effects" width="1402" height="1122"></canvas><div class="hotel-road-sign"><span>150 Rooms</span><span>The Grand English Hotel</span></div><div class="hotel-companion-portrait"><canvas width="74" height="96"></canvas><span>Eddie</span></div></div>`;}
...
export function mountHotel(root,reduced){
 let disposed=false,elapsed=0,last=0,lastPaint=-Infinity,scenery,effects,ready=false,lastRoom='',lastCharacter='';
 ...
 function paint(force=false){if(!ready)return;const t=reduced.matches?0:elapsed;if(force||(!reduced.matches&&t-lastPaint>=1/30)){scenery?.paint(t,reduced.matches);effects.paint(t,reduced.matches);lastPaint=t;}}
 const reference=new Image(),restoration=new Image();
 Promise.all([reference,restoration].map((img,i)=>new Promise((resolve,reject)=>{img.onload=resolve;img.onerror=reject;img.src=new URL(ART+(i?'restoration.png':'reference.png'),import.meta.url).href;}))).then(()=>{
  ...
  effects=createHotelEffects(fx,reference);ready=true;root.dataset.hotelReady='true';paint(true);
