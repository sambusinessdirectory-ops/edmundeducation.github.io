-- Edmund Education Sentence Structure: enable lessons ss40 through ss70.
--
...
begin
  if p_lesson_id !~ '^ss([1-9]|[1-6][0-9]|70)$'
    or p_result is null
...
begin
  if p_bookmarks is null
    ...
    or jsonb_array_length(p_bookmarks) > 4000
  ...
  loop
    if jsonb_typeof(v_item) <> 'object'
      ...
      or coalesce(v_item ->> 'lessonId', '') !~ '^ss([1-9]|[1-6][0-9]|70)$'
...
as $$
begin
  if p_offset not between 0 and 4000
...
alter table public.sentence_structure_attempts
  ...
  check (lesson_id ~ '^ss([1-9]|[1-6][0-9]|70)$') not valid;
...
begin
  ...
  if p_id is null
    or p_lesson_id !~ '^ss([1-9]|[1-6][0-9]|70)$'
