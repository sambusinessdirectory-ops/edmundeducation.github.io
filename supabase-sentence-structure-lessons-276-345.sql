-- Edmund Education Sentence Structure: enable lessons ss276 through ss345.
--
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('sentence-structure-lessons-276-345', 0)
);

do $$
begin
  if to_regclass('public.sentence_structure_attempts') is null
    or to_regclass('public.sentence_structure_bookmarks') is null
  then
    raise exception 'Missing dependency: apply supabase-sentence-structure.sql first';
  end if;
end;
$$;
...
begin
  if p_lesson_id !~ '^ss([1-9]|[1-9][0-9]|[12][0-9]{2}|3[0-3][0-9]|34[0-5])$'
    or p_result is null
...
begin
  if p_bookmarks is null
    ...
    or jsonb_array_length(p_bookmarks) > 20000
    or octet_length(p_bookmarks::text) > 2097152
  then
    return false;
  end if;
  ...
  loop
    if jsonb_typeof(v_item) <> 'object'
      ...
      or coalesce(v_item ->> 'lessonId', '') !~ '^ss([1-9]|[1-9][0-9]|[12][0-9]{2}|3[0-3][0-9]|34[0-5])$'
...
-- Keep the legacy readers available during the rolling Worker deployment, but
-- raise their old 200-row cap to PostgREST's default maximum.
...
as $$
begin
  if p_offset not between 0 and 20000
...
alter table public.sentence_structure_attempts
  ...
  check (lesson_id ~ '^ss([1-9]|[1-9][0-9]|[12][0-9]{2}|3[0-3][0-9]|34[0-5])$') not valid;
...
begin
  ...
  if p_id is null
    or p_lesson_id !~ '^ss([1-9]|[1-9][0-9]|[12][0-9]{2}|3[0-3][0-9]|34[0-5])$'
...
-- CREATE OR REPLACE preserves ACLs on a normal upgrade, but a partial legacy
-- install may not already have these functions. Reset every recreated RPC to
-- fail closed before granting the Worker service role explicitly.
...
revoke all on function public.sentence_structure_list_bookmarks(uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.sentence_structure_list_bookmarks_page(
...
grant execute on function public.sentence_structure_list_bookmarks(uuid)
  to service_role;
grant execute on function public.sentence_structure_list_bookmarks_page(
  uuid, integer, integer
)
  to service_role;
