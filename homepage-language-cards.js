(function(){
  ...
  const languages={en:'原系統',it:'Italian',fr:'French',de:'German',es:'Spanish',ja:'Japanese',ko:'Korean'};
  for(const [code,label] of Object.entries(languages))if(code!=='en'){names.flashcard[code]=`${label}<br>Flashcard`;names.writing[code]=`${label}<br>Practice System`;}
  ...
  document.querySelectorAll('a.category.writing-system-card,a.category.flashcard-card').forEach(card=>{
    ...
    let saved;try{saved=localStorage.getItem('homepage-language:'+kind);}catch{}select(card,kind,Object.hasOwn(languages,saved)?saved:'en',false);
    ...
    button.addEventListener('click',e=>{
      ...
      for(const code of Object.keys(languages)){const b=document.createElement('button');b.type='button';b.setAttribute('role','menuitemradio');b.setAttribute('aria-checked',String(card.dataset.cardLanguage===code));b.textContent=code==='en'?'原系統':`${languages[code]} ${kind==='flashcard'?'Flashcard':'Practice System'}`;b.addEventListener('click',()=>{close();document.querySelectorAll(kind==='flashcard'?'a.category.flashcard-card':'a.category.writing-system-card').forEach(c=>select(c,kind,code));try{localStorage.setItem('homepage-language:'+kind,code);}catch{}button.focus();});menu.append(b);}
