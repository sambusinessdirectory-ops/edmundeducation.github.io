import {HOTEL_ART,HOTEL_FLAGS,HOTEL_PLANTS,HOTEL_TREES,HOTEL_LIGHTS,HOTEL_SILHOUETTE,hotelTrainPose,hotelFlagOffset,hotelLightLevel} from './sentence-structure-hotel-geometry.mjs?v=20260914-hotel1';
...
function restorePaint(ctx,reference){
 ...
 for(const [x,y,w,h] of HOTEL_LETTERING_REPAIRS){
  ...
  for(let row=0;row<h;row++)for(let col=0;col<w;col++){
   ...
   const feather=Math.min(1,col/2,(w-1-col)/2,row/2,(h-1-row)/2);
...
function masks(plate){
 ...
 for(let y=0;y<H;y++)for(let x=0;x<W;x++){
  ...
  for(const a of HOTEL_PLANTS)if(y<a[4])plant=Math.max(plant,ellipse(x,y,a)*Math.min(1,(a[4]-y)/15));
  for(const a of HOTEL_TREES)if(y<a[4])tree=Math.max(tree,ellipse(x,y,a)*Math.min(1,(a[4]-y)/30));
...
const fragment=`precision highp float;uniform sampler2D u_image;uniform sampler2D u_mask;uniform float u_time;uniform float u_motion;varying vec2 v_uv;
void main(){vec4 mask=texture2D(u_mask,v_uv);vec2 p=v_uv*vec2(1402.,1122.);float t=u_time;
 float dx=sin(t*.82+p.y*.025)*1.9*mask.r+sin(t*.46+p.x*.031)*2.5*mask.g;
...
export function createHotelScenery(canvas,plate){
 ...
 const time=gl.getUniformLocation(program,'u_time'),motion=gl.getUniformLocation(program,'u_motion');canvas.dataset.renderer='webgl';
 return {paint(t,still=false){gl.viewport(0,0,canvas.width,canvas.height);gl.uniform1f(time,t);gl.uniform1f(motion,still?0:1);gl.drawArrays(gl.TRIANGLES,0,6);canvas.dataset.time=String(t);},destroy(){textures.forEach(x=>gl.deleteTexture(x));shaders.forEach(x=>gl.deleteShader(x));gl.deleteBuffer(buffer);gl.deleteProgram(program);gl.getExtension('WEBGL_lose_context')?.loseContext();}};
}
...
export function createHotelEffects(canvas,reference){
 const ctx=canvas.getContext('2d');
 const train=cutout(reference,TRAIN_OUTLINE);
 ...
 return {paint(t,still=false){
  ...
  const pose=still?hotelTrainPose(14):hotelTrainPose(t);ctx.save();exteriorClip(ctx);ctx.drawImage(train,pose.dx,pose.dy);ctx.restore();
  if(!still){ctx.save();exteriorClip(ctx);for(let i=0;i<235;i++){const depth=random(i+700),speed=13+depth*24,x=(random(i+30)*W+Math.sin(t*.25+i)*8+t*(2+depth*2))%(W+12),y=(random(i+500)*H+t*speed)%(H+12)-6;ctx.globalAlpha=.25+depth*.5;ctx.fillStyle='#fff8ed';ctx.beginPath();ctx.ellipse(x,y,.65+depth*1.45,1+depth*1.9,.18,0,Math.PI*2);ctx.fill();}ctx.restore();}
