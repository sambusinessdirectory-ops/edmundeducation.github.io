...
export const LOUNGE_PLANTS=[{x:120,y:655,h:315},{x:1460,y:765,h:300},{x:270,y:850,h:170}];
...
export function mountLounge(root,reduced){const canvas=root.querySelector('.lounge-motion'),ctx=canvas.getContext('2d');let ready,dead=false,elapsed=0,last=0,painted=-1;
 ...
 function render(seconds){if(!ready||dead)return;ctx.clearRect(0,0,1600,1200);const motion=loungeMotion(seconds),s=ready.sprites.bass,bh=260,bw=bh*s.width/s.height,bx=1085,by=300;
  const bar=ready.sprites.bar,mic=ready.sprites.mic;ctx.drawImage(bar,1220,8,370,370*bar.height/bar.width);const mh=215,mw=mh*mic.width/mic.height;shadow(917,302,mw*.55);ctx.drawImage(mic,917-mw/2,300-mh,mw,mh);
