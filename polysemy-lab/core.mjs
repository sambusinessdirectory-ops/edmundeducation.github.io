...
export function replay(events){
 ...
 answers.forEach((a,i)=>{seen.add(a.question);attempts.set(a.question,Math.max(attempts.get(a.question)||0,a.round));if(isCorrectAnswer(questionMap.get(a.question),a.choice)){correct.add(a.question);pending.delete(a.question);streak++;bestStreak=Math.max(bestStreak,streak);}else{streak=0;const gap=5+Math.floor(random(run+':'+a.question+':'+a.round)()*2);pending.set(a.question,i+gap+1);}});
...
export function summary(events){events=events.filter(e=>(e.module||'show')===showModule.id);const seen=new Set(events.filter(e=>e.kind==='view').map(e=>e.sense)),mastered=new Set(events.filter(e=>e.kind==='answer'&&isCorrectAnswer(questionMap.get(e.question),e.choice)).map(e=>e.question));return {seen,mastered};}
...
export function dailyAnswers(events){const rows=new Map();for(const e of [...events].sort((a,b)=>a.at.localeCompare(b.at))){if(e.kind!=='answer'||!allQuestionMap.has(e.question))continue;const date=hkDate(e.at);if(!rows.has(date))rows.set(date,{date,attempts:0,correct:0});rows.get(date).attempts++;}
 const first=new Map();for(const e of [...events].sort((a,b)=>a.at.localeCompare(b.at))){if(e.kind==='answer'&&isCorrectAnswer(allQuestionMap.get(e.question),e.choice)&&!first.has(e.run+e.question)){first.set(e.run+e.question,true);rows.get(hkDate(e.at)).correct++;}}
