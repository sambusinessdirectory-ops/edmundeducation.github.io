import {createMedia} from './media.mjs?v=20260923-polysemy-127';
import {modules,moduleMap,allQuestionMap,selectModule,showModule,questions,senses,questionMap,orderedOptions,replay,summary,dailyAnswers,hkDate,esc,highlighted} from './core.mjs?v=20260923-polysemy-127';
...
function maskChineseTranslation(q){
 ...
 for(const candidate of candidates){if(zh.includes(candidate))return zh.replace(candidate,"____");}
...
function renderPractice(){
 ...
 const explanationParts=String(q.explanation||"").match(/[^，、；。！？,.!?;]+[，、；。！？,.!?;]?/gu)?.map(part=>part.trim()).filter(Boolean)||[String(q.explanation||"")];
 root.innerHTML=`<div class="practice-toolbar"><p>第 ${pos+1} 題 · 隨機練習${(feedback?.review??state.review)?' · 重溫題目':(feedback?.filler??state.filler)?' · 穿插鞏固':''}</p><button data-reference>查看詞義總覽</button></div><div class="progress-wrap"><label for="practice-progress">本次練習已答對 ${state.correct.size} / ${questions.length} 題</label><progress id="practice-progress" max="${questions.length}" value="${state.correct.size}"></progress></div><section class="polysemy-streak" aria-live="polite" data-streak-stage="${Math.min(4,Math.floor(state.streak/2))}"><div class="polysemy-streak-copy"><img class="polysemy-streak-fire" src="/assets/schedule/day-streak-fire.gif" alt="" aria-hidden="true"><div><strong>連續答對 ${state.streak} 題</strong><small>最高連勝 ${state.bestStreak} 題</small></div></div><div class="polysemy-streak-eddy" role="group" aria-label="Eddie 連勝鼓勵"><span class="eddy-streak-sprite" role="img" aria-label="Eddie 為你的連勝打氣"></span><span class="eddy-streak-cheer">${state.streak>=10?"太厲害了！繼續衝！":state.streak>=5?"連中五題！做得好！":state.streak>=2?"答得好，繼續！":"你做得到！"}</span></div></section><article class="question-card ${feedback?(feedback.choice===q.sense?"is-correct":"is-wrong"):""}"><p class="eyebrow">${q.passage?'ORIGINAL CONTEXT / 原句挑戰':'READ THE CONTEXT / 閱讀語境'}</p><h2 class="sentence" tabindex="-1" lang="en">${highlighted(q.en)}</h2><div data-sentence-media></div><p class="question-translation" lang="zh-Hant"><strong>中文翻譯：</strong>${esc(maskChineseTranslation(q))}</p><p class="masked" lang="en">${esc(q.masked)}</p><p>這句中的標示詞語是甚麼意思？</p><div class="answer-grid" role="group" aria-label="六個意思選項">${orderedOptions(q,round).map((id,i)=>`<button data-answer="${id}" ${feedback?'disabled':''} class="${feedback?(id===q.sense?'correct':id===feedback.choice?'wrong':''):''}"><span class="letter">${String.fromCharCode(65+i)}</span><span>${esc(senses.get(id).title)}${feedback&&id===q.sense?' ✓':''}</span></button>`).join('')}</div>${feedback?`<section class="feedback ${feedback.choice===q.sense?'':'wrong'}" role="status"><h3 class="feedback-step" style="--feedback-step:0">${feedback.choice===q.sense?'答對了！':'再留意句子中的線索。'}</h3><div class="feedback-step" style="--feedback-step:1"><strong>正確意思</strong><p>${esc(senses.get(q.sense).title)}；${esc(senses.get(q.sense).zh)}</p></div><div class="feedback-explanation"><strong class="feedback-step" style="--feedback-step:2">語境解釋</strong>${explanationParts.map((part,index)=>`<p class="feedback-step feedback-explanation-fragment" style="--feedback-step:${3+index}">${esc(part)}</p>`).join('')}</div><div class="feedback-step" style="--feedback-step:${3+explanationParts.length}"><strong>完整翻譯</strong><p>${esc(q.zh)}</p></div>${feedback.choice!==q.sense?`<small class="feedback-step" style="--feedback-step:${4+explanationParts.length}">這題會在另外 5–6 題之後再出現。綠色選項為正確答案。</small>`:''}</section><button class="primary" data-next>${state.complete?'查看結果':'下一題'}</button>`:''}</article><p class="timer-note">可以隨時查看詞義總覽。進度會儲存；隱藏分頁或閒置 10 分鐘後，計時會暫停。</p>`;
