import {HOTEL_OFFSET,HOTEL_HEIGHT,hotelPositions,hotelArrivalIndex} from './sentence-structure-hotel-geometry.mjs?v=20260914-hotel1';
import {hotelTerrain,decorateHotelDoors,mountHotel} from './sentence-structure-hotel.mjs?v=20260914-hotel1';
...
import { AUTUMN_OFFSET, realmsPath, realmsStep } from './sentence-structure-realms-navigation.mjs?v=20260914-hotel1';
...
import { dreamTerrain,decorateDreamStones,mountDream } from './sentence-structure-dream.mjs?v=20260912-toy1';
...
export function sentenceRealmPositions(lessons){return [
 ...
 ...toyPositions(lessons.slice(120,150)),
 ...hotelPositions(lessons.slice(150,180))
];}
let ids=0;
...
function mount(root,reduced){
 ...
 decorateZenStones(root);decorateDreamStones(root);decorateToyStones(root);decorateHotelDoors(root);
 const coast=SENTENCE_COAST.mount(root,reduced,{stoneSelector:'.expression-map-stone:not([data-autumn]):not([data-zen]):not([data-dream]):not([data-toy]):not([data-hotel])',label:'句型海岸、秋林、庭園、夢境、玩具與酒店地圖，第 1 至 180 課；可穿過雲霧自由來往，沿橋過河，點選石階或使用方向鍵 / WASD 走動。'});
 const zen=mountZen(root,reduced),dream=mountDream(root,reduced),toy=mountToy(root,reduced),hotel=mountHotel(root,reduced);
 ...
 function update(){
  ...
  root.dataset.currentRealm=y>=HOTEL_OFFSET?'hotel':y>=TOY_OFFSET?'toy':y>=DREAM_OFFSET?'dream':y>=ZEN_OFFSET?'zen':y>=AUTUMN_OFFSET?'autumn':'shore';zen.update();dream.update();toy.update();hotel.update();
 }
 function draw(now){if(disposed)return;coast?.draw(now);zen.draw(now);dream.draw(now);toy.draw(now);hotel.draw(now);if(last)elapsed+=Math.min(100,Math.max(0,now-last))/1000;last=now;rig?.paint(canvas,rabbitMotion(reduced.matches?0:elapsed));update();}
 viewport.addEventListener('scroll',update,{passive:true});
 return {draw,update,destroy(){disposed=true;image.onload=null;viewport.removeEventListener('scroll',update);coast?.destroy();zen.destroy();dream.destroy();toy.destroy();hotel.destroy();}};
}
export const SENTENCE_REALMS=Object.freeze({
 id:'sentence-shore',title:'',kicker:'THE SENTENCE JOURNEY',width:1600,height:HOTEL_OFFSET+HOTEL_HEIGHT,
 ...
 cameraSpaceHeight:({scale,height,point})=>Math.max((HOTEL_OFFSET+HOTEL_HEIGHT)*scale,point.y>=HOTEL_OFFSET?HOTEL_OFFSET*scale+height:0),
 arrivalIndex:hotelArrivalIndex,navigation:{path:realmsPath,step:realmsStep},cameraTop:({point,scale,height,zoom})=>zoom===1&&height>=600?(point.y<625?0:point.y>=TOY_OFFSET&&point.y<TOY_OFFSET+930&&(point.y-TOY_OFFSET-100)*scale<height-50?(TOY_OFFSET+100)*scale:point.y>=DREAM_OFFSET&&point.y<DREAM_OFFSET+850?(DREAM_OFFSET+145)*scale:point.y>=ZEN_OFFSET&&point.y<ZEN_OFFSET+430?(ZEN_OFFSET+15)*scale:point.y*scale-height*.4):point.y*scale-height*.4
});
