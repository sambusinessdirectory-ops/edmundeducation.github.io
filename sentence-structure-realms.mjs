...
import { AUTUMN_OFFSET, realmsPath, realmsStep } from './sentence-structure-realms-navigation.mjs?v=20260912-autumn1';
...
function terrain(nodes,lessons){
 const autumn=nodes.slice(30).map(p=>({...p,y:p.y-AUTUMN_OFFSET}));
...
function mount(root,reduced){
 root.querySelectorAll('.expression-map-stone').forEach((stone,i)=>{if(i>=30){stone.dataset.autumn='true';stone.insertAdjacentHTML('afterbegin',`<svg class="autumn-stone-art" viewBox="20 166 1570 710" aria-hidden="true"><image href="${ART}stone.webp" width="1611" height="976"/></svg>`);}});
 const coast=SENTENCE_COAST.mount(root,reduced,{stoneSelector:'.expression-map-stone:not([data-autumn])',label:'句型海岸與秋林地圖，第 1 至 60 課；可穿過雲霧自由來往，沿橋過河，點選石階或使用方向鍵 / WASD 走動。'});
 ...
 function update(){
  ...
  leaves.style.clipPath=`inset(${Math.max(0,Math.min(viewport.clientHeight,top))}px 0 0)`;
  leaves.style.opacity=String(top<viewport.clientHeight?1:0);
  flag.classList.toggle('is-autumn',Number(flag.dataset.flagLevel?.slice(2))>30);
  root.dataset.currentRealm=parseFloat(root.querySelector('.expression-map-horse').style.top)+12>=AUTUMN_OFFSET?'autumn':'shore';
 }
 function draw(now){if(disposed)return;coast?.draw(now);if(last)elapsed+=Math.min(100,Math.max(0,now-last))/1000;last=now;rig?.paint(canvas,rabbitMotion(reduced.matches?0:elapsed));update();}
 viewport.addEventListener('scroll',update,{passive:true});
 return {draw,update,destroy(){disposed=true;image.onload=null;viewport.removeEventListener('scroll',update);coast?.destroy();}};
}
export const SENTENCE_REALMS=Object.freeze({
 id:'sentence-shore',title:'海岸與秋林之旅',kicker:'THE SENTENCE JOURNEY',width:1600,height:3900,
 cameraPadding:{left:800,right:800},minimumZoom:.5,positions:sentenceRealmPositions,terrain,mount,overlay,
 navigation:{path:realmsPath,step:realmsStep},cameraTop:({point,scale,height,zoom})=>zoom===1&&height>=600&&point.y<625?0:point.y*scale-height*.4
});
