import {HOTEL_OFFSET,HOTEL_HEIGHT,hotelPositions,hotelArrivalIndex,hotelJourneyDuration} from './sentence-structure-hotel-geometry.mjs?v=20260914-hotel3b';
...
import { levelPositions } from './common-expression-map.mjs?v=20260923-olive-tee-leg-gap1';
...
export const SENTENCE_REALMS=Object.freeze({
 ...
 arrivalIndex:hotelArrivalIndex,navigation:{path:realmsPath,step:realmsStep,duration:hotelJourneyDuration},cameraTop:({point,scale,height,zoom})=>zoom===1&&height>=600?(point.y<625?0:point.y>=TOY_OFFSET&&point.y<TOY_OFFSET+930&&(point.y-TOY_OFFSET-100)*scale<height-50?(TOY_OFFSET+100)*scale:point.y>=DREAM_OFFSET&&point.y<DREAM_OFFSET+850?(DREAM_OFFSET+145)*scale:point.y>=ZEN_OFFSET&&point.y<ZEN_OFFSET+430?(ZEN_OFFSET+15)*scale:point.y*scale-height*.4):point.y*scale-height*.4
});
