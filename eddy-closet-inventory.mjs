import {COSMETICS,cosmeticAsset,cosmeticsState,equipCosmetic,equipOutfit,clearCosmetics,saveAvatar,restoreCosmetics,subscribeCosmetics,toggleOutfitFavorite} from './eddy-cosmetics.mjs?v=20260915-closet2';
export function mountClosetInventory(host,signal){
 ...
 sets.addEventListener('click',event=>{const heart=event.target.closest('[data-favorite]');if(heart&&!busy){busy=true;heart.disabled=true;toggleOutfitFavorite(heart.dataset.favorite).then(()=>{status.textContent='Favorites saved · 已儲存最愛';}).catch(e=>{status.textContent=e.message;}).finally(()=>{busy=false;heart.disabled=false;render();});return;}const button=event.target.closest('[data-outfit]');if(button&&!busy){equipOutfit(button.dataset.outfit);void save();}},{signal});
