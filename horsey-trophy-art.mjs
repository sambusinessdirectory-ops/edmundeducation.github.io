...
for(const name of ['phoebe','elsie'])TROPHY_ART[name].metals={silver:BASE+'silver-'+name+'-map-v1.webp',bronze:BASE+'bronze-'+name+'-map-v1.webp'};
let selected='eddy';
...
export function syncCompanionTrophies(scope,character=selected){
 ...
 for(const img of scope.querySelectorAll('img[data-trophy-art-view],img[src*="/rewards/golden-"]')){
  ...
  if(button){button.dataset.trophyCharacter=character;const label=button.getAttribute('aria-label');if(label)button.setAttribute('aria-label',label.replace(/Eddie|Phoebe|Elsie/g,{eddy:'Eddie',phoebe:'Phoebe',elsie:'Elsie'}[character]));}
 }
}
