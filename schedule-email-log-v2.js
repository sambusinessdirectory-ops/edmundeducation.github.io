...
async function refresh(){
 ...
 for(const item of data.logs||[]){
  ...
  for(const point of [...(item.checkpoints||[])].sort((a,b)=>new Date(a.time)-new Date(b.time))){const li=element('li',`${when(point.time)} · ${EMAIL_STAGES[point.stage]||point.stage} [${point.outcome}]`);li.append(element('pre',JSON.stringify(point.details,null,2)));timeline.append(li);}
 ...
 document.querySelector('[data-requests]').replaceChildren(...[...(data.requests||[])].sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)).map(r=>details(`${when(r.created_at)} · ${EMAIL_STAGES[r.stage]||r.stage} · ${r.outcome}`,`要求 ID: ${r.request_id}\n${JSON.stringify(r.details,null,2)}`)));
