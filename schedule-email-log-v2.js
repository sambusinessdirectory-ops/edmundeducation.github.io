...
async function refresh(){
 ...
 for(const item of data.logs||[]){
  ...
  row.append(element('td',when(item.created_at)),element('td',item.kind==='writing_feedback_ready'?'評改完成通知（學生）':item.kind==='writing_submission'?'寫作提交通知（管理員）':item.kind?.startsWith('visitor')?`${item.kind==='visitor_confirmation'?'訪客確認信':'訪客更新'} ${EMAIL_TOPICS[item.topic]?.title||''}`:`學生訊息 ${item.template_slot??'已刪除'}`),element('td',`${item.recipient_name||'—'} · ${item.recipient_email}`));
